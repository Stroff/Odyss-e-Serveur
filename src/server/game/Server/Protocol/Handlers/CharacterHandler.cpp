...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
    ...
    {
        QueryResult recupresult = RecupDatabase.PQuery("SELECT * FROM %s WHERE id_compte = %u AND classe = 6 AND etat_recuperation=0 AND etat_ouverture=10", sWorld->GetRecupTable(), GetAccountId());
        if (recupresult && (recupresult->GetRowCount() != 0)) {}
        else 
        {
   ...
   if (sWorld->getBoolConfig(CONFIG_RECUP_ENABLE) == true)
    {
        QueryResult recupresult1 = RecupDatabase.PQuery("SELECT * FROM %s WHERE id_compte = %u AND classe = 6 AND etat_recuperation=0 AND etat_ouverture=10", sWorld->GetRecupTable(), GetAccountId());
        if (recupresult1 && (recupresult1->GetRowCount() != 0))
        {
            sLog->outSimple("Création de DK pour récupération (Compte %u)", GetAccountId());
        }
        else
        {   
    ...
    if (sWorld->getBoolConfig(CONFIG_RECUP_ENABLE) == true)
    {
        QueryResult recupresult2 = RecupDatabase.PQuery("SELECT * FROM %s WHERE id_compte = %u AND classe = 6 AND etat_recuperation=0 AND etat_ouverture=10", sWorld->GetRecupTable(), GetAccountId());
        if (recupresult2 && (recupresult2->GetRowCount() != 0))
        {
            RecupDatabase.PQuery("UPDATE %s SET id_perso_cible = %u WHERE id_compte = %u AND classe = 6 AND etat_recuperation=0 AND etat_ouverture=10", sWorld->GetRecupTable(), pNewChar->GetGUID(), GetAccountId());
        }
    }	
    charcount+=1;
...
void WorldSession::HandleCharRaceChange(WorldPacket& recv_data)
{
    ...
    /// Code de la fonction de customisation
    ...
    Player::Customize(guid, gender, skin, face, hairStyle, hairColor, facialHair, true);
    CharacterDatabase.PExecute("UPDATE characters SET race = '%u', at_login = at_login & ~ %u WHERE guid ='%u'", race, AT_LOGIN_CHANGE_RACE, GUID_LOPART(guid));
    CharacterDatabase.PExecute("DELETE FROM character_declinedname WHERE guid ='%u'", GUID_LOPART(guid));
    CharacterDatabase.PExecute("DELETE FROM character_skills WHERE guid ='%u' AND skill IN(98, 109, 111, 113, 137, 141, 313, 315, 673, 759)", GUID_LOPART(guid));

    /// Envoi du résulat du succès (utilisation de l'opcode serveur de faction; n'existant pas pour le changement de race)
    WorldPacket data(SMSG_CHAR_FACTION_CHANGE, 1+8+(name.size()+1)+7);
    data << uint8(RESPONSE_SUCCESS);
    data << uint64(guid);       // guid
    data << name;               // nom
    data << uint8(gender);      // sexe
    data << uint8(skin);        // skin général
    data << uint8(face);        // tête
    data << uint8(hairStyle);   // type de cheveux
    data << uint8(hairColor);   // couleur des cheveux
    data << uint8(facialHair);  // pilosité
    data << uint8(race);        // le plus important: la race
    SendPacket(&data);
}
...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(QueryResult result, std::string newname)
{
    ...
    WorldSession * session = sWorld->FindSession(GetAccountId());
    if (!session)
            return;
	...
	RecupDatabase.PExecute("INSERT INTO log_rename(guid, accountid, oldname, newname, date) VALUES(%u, %u, '%s', '%s', CURDATE())", guidLow, session->GetAccountId(), oldname.c_str(), newname.c_str());
...
void WorldSession::HandleCharCustomize(WorldPacket& recv_data)
{
    ...
    sObjectMgr->GetPlayerNameByGUID(guid, newname);
    ...
    SendPacket(&data);
        ...
        sLog->outSimple("Customisation compte %d (IP: %s), Personnage[%s] (guid:%u) terminée)", GetAccountId(), GetRemoteAddress().c_str(), newname.c_str(), GUID_LOPART(guid));
}
