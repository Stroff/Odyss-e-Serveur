    ...
    if (m_MirrorTimerFlags & (UNDERWATER_INLAVA|UNDERWATER_INSLIME))
    {
        ...
        else
        {
            ...
            if (m_MirrorTimer[FIRE_TIMER] < 0)
            {
                ...
                // TODO: Check this formula
                uint32 damage =0;
                // insensible au dégats de zone (cf : bubulle)
                if (!HasAura(642))
...
void Player::Update(uint32 p_time)
{
    ...
    if (GetAreaId() == 4281)  
    {
        if (FindNearestCreature(438700, 1) && GetPositionZ() >= 410)
            TeleportTo(0, 2401.41f, -5633.91f, 377.02f, 3.664271f);
        if (FindNearestCreature(438700,1) && GetPositionZ() <= 390)
            TeleportTo(0, 2401.41f, -5633.91f, 420.67f, 3.664271f);
    } 

    if (GetAreaId() == 4342)
    {
        if (FindNearestCreature(438700, 1) && GetPositionZ() >= 410)
            TeleportTo(609, 2401.41f, -5633.91f, 377.02f, 3.664271f);
        if (FindNearestCreature(438700,1) && GetPositionZ() <= 390)
            TeleportTo(609, 2401.41f, -5633.91f, 420.67f, 3.664271f);
    }		
...
void Player::CheckDuelDistance(time_t currTime)
{
    ...
    // Hack odyssée : Zone de duel agrandie
...
{
    /* Odyssée : On force systématiquement la réactualisation
...
void Player::Customize(uint64 guid, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, bool change_gender)
{
    //                                                     0
    ...
    if (change_gender)
        ...
        CharacterDatabase.PExecute("UPDATE characters SET playerBytes = '%u', playerBytes2 = '%u' WHERE guid = '%u'", skin | (face << 8) | (hairStyle << 16) | (hairColor << 24), player_bytes2, GUID_LOPART(guid));
}
...
void Player::ActivateSpec(uint8 spec)
{
    ...
    if (getClass() == CLASS_WARRIOR) //Agma sous fiffre debug spé combat war, merci à Hugz et aux Pokédex.
    {
        Item *pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND); //Main droite
        if (pItem)
        {
            ItemPosCountVec dest;
            uint8 msg = CanStoreItem(NULL_BAG, NULL_SLOT, dest, pItem, false);
            if (msg == EQUIP_ERR_OK)
            {
            ...
            else
            {
                ...
                SQLTransaction sTrans = CharacterDatabase.BeginTransaction();
                pItem->DeleteFromInventoryDB(sTrans);
                pItem->SaveToDB(sTrans);
                CharacterDatabase.CommitTransaction(sTrans);

                std::string subject = GetSession()->GetTrinityString(LANG_NOT_EQUIPPED_ITEM);
                MailDraft(subject, "There were problems with equipping one or several items").AddItem(pItem).SendMailTo(sTrans, this, MailSender(this, MAIL_STATIONERY_GM), MAIL_CHECK_MASK_COPIED);
				CharacterDatabase.CommitTransaction(sTrans);
            }
        }

        pItem = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND); //Main gauche
        if (pItem)
        {
            ItemPosCountVec dest;
            uint8 msg = CanStoreItem(NULL_BAG, NULL_SLOT, dest, pItem, false);
    ...
    {
        RemoveAurasDueToSpell(28176); //Gangrarmure
        RemoveAurasDueToSpell(28189);
        RemoveAurasDueToSpell(47892);
        RemoveAurasDueToSpell(47893);

        RemoveAurasDueToSpell(706); //Armure Démoniaque
        RemoveAurasDueToSpell(1086);
        RemoveAurasDueToSpell(11733);
        RemoveAurasDueToSpell(11734);
        RemoveAurasDueToSpell(11735);
        RemoveAurasDueToSpell(27260);
        RemoveAurasDueToSpell(47793);
        RemoveAurasDueToSpell(47889);
    }
...
void Player::RefundItem(Item *item)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_EXTENDED_COST_REQUIREMENTS; ++i)
    {
        ...
        if (count && itemid)
        {
            ...
            int32 price = it->GetPaidMoney() * count;
            QueryResult lolilol = WorldDatabase.PQuery("SELECT ExtendedCost FROM npc_vendor WHERE item = %i LIMIT 1", itemid);
            uint32 ExtendedCost = 0;
            if (lolilol)
                ExtendedCost = (lolilol->Fetch())[0].GetUInt32();
            else
                ExtendedCost = 0;
            if (it->GetProto()->Flags & ITEM_PROTO_FLAG_REFUNDABLE && iece->ID && it->GetProto()->GetMaxStackSize() == 1)
            {
                it->SetFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_REFUNDABLE);
                it->SetRefundRecipient(GetGUIDLow());
                it->SetPaidMoney(price);
                it->SetPaidExtendedCost(ExtendedCost);
                it->SaveRefundDataToDB();
                AddRefundReference(it->GetGUIDLow());
            }			
        }
...
/*
 * Renvoit le nombre d'emplacement avec un item niveau uLevel
 */
uint32 Player::GetAverageItemLevelOdyssee(uint32 uLevel) const
{
    uint32 count = 0;
    ...
    for (int i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; ++i)
    {
        ...
        if (m_items[i] && m_items[i]->GetProto() && m_items[i]->GetProto()->ItemLevel >= uLevel)
            count++;
    }
