...
void Unit::SendMonsterMoveTransportGormokk(Unit *vehicleOwner, uint32 VehicleId)
{
    WorldPacket data(SMSG_PLAYER_VEHICLE_DATA, vehicleOwner->GetPackGUID().size()+4);
    data.appendPackGUID(vehicleOwner->GetGUID());
    data << uint32(VehicleId);
    SendMessageToSet(&data,true);

    data.Initialize(SMSG_ON_CANCEL_EXPECTED_RIDE_VEHICLE_AURA, 0);
    vehicleOwner->ToPlayer()->GetSession()->SendPacket(&data);
}
...
void Unit::DealDamageMods(Unit *pVictim, uint32 &damage, uint32* absorb)
{
    ...
    if (pVictim != this && IsControlledByPlayer() && pVictim->IsControlledByPlayer())
    {
        ...
        if ((ToPlayer() && ToPlayer()->duel && ToPlayer()->duel->opponent == pVictim)  // joueur eux meme
             || (isPet() && GetOwner() && GetOwner()->ToPlayer() && GetOwner()->ToPlayer()->duel && GetOwner()->ToPlayer()->duel->opponent == pVictim) // les degats des pets
             || (pVictim->isPet() && pVictim->GetOwner() && pVictim->GetOwner()->ToPlayer() && pVictim->GetOwner()->ToPlayer()->duel && pVictim->GetOwner()->ToPlayer()->duel->opponent == this) // on peut attaquer les pets
             || (GetCharmerOrOwnerPlayerOrPlayerItself() && GetCharmerOrOwnerPlayerOrPlayerItself()->duel && GetCharmerOrOwnerPlayerOrPlayerItself()->duel->opponent == pVictim)    // les degats des pets temporaires
             || (pVictim->GetCharmerOrOwnerPlayerOrPlayerItself() && pVictim->GetCharmerOrOwnerPlayerOrPlayerItself()->duel && pVictim->GetCharmerOrOwnerPlayerOrPlayerItself()->duel->opponent == this) // on peut attaquer les pets temporaires
           )
            {}
        else if (area && area->IsSanctuary())      //sanctuary
        {
            if (absorb)
                *absorb += damage;
            damage = 0;
...
uint32 Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo, bool durabilityLoss)
{
    ...
    return DealDamage(pVictim, damageInfo->damage, &cleanDamage, SPELL_DIRECT_DAMAGE, SpellSchoolMask(damageInfo->schoolMask), spellProto, durabilityLoss);
}
...
uint32 Unit::DealMeleeDamage(CalcDamageInfo *damageInfo, bool durabilityLoss)
{
    Unit *pVictim = damageInfo->target;
    ...
    uint32 damageToReturn = DealDamage(pVictim, damageInfo->damage, &cleanDamage, DIRECT_DAMAGE, SpellSchoolMask(damageInfo->damageSchoolMask), NULL, durabilityLoss);
...
uint32 Unit::CalcSpellResistance(Unit * pVictim, SpellSchoolMask schoolMask, bool binary, SpellEntry const * spellProto) const
{
    ...
    if (binary &&           // No partial resists for binary spells  
        pVictim != this &&                              // si on affecte un sort sur soit il ne peut pas resist (ex : fouet mental)
        !(this->IsFriendlyTo(pVictim)) &&               // si on fais un spell sur un ami on ne peut pas resist
        spellProto->Id != 28730)                        // le torrent arcanique ne peut pas resist
    {
        if( (pVictim->GetTypeId() == TYPEID_PLAYER ||   // "Lutte" contre les fulls resist en PvE
            (pVictim->isPet() && pVictim->GetOwner() && pVictim->GetOwner()->ToPlayer()) ||     // pour faire des resists sur les pets
...
void Unit::CalcAbsorbResist(Unit * pVictim, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 * absorb, uint32 * resist, SpellEntry const * spellInfo, int32 calc_resist)
{
    ...
    int32 RemainingDamage = dmgInfo.GetDamage();
...
void Unit::AttackerStateUpdate (Unit *pVictim, WeaponAttackType attType, bool extra)
{
    ...
    else
    {
        ...
        damageInfo.damage = DealMeleeDamage(&damageInfo,true);
...
//              coup bas
		spell->Id == 1833 || 
//	            Aiguillon perfide rang
//             1                     2
		spell->Id == 408 || spell->Id == 8643 ||
//										traquenard rang 
//              1                    2                       3                   4                    5
		spell->Id == 9005 || spell->Id == 9823 || spell->Id == 9827 || spell->Id == 27006 || spell->Id == 49803 ||
//				Estropier (drood) rang
//              1                    2
		spell->Id == 22570 || spell->Id == 49808))
...
uint32 Unit::CalcMagicSpellHitChance(Unit * pVictim, SpellSchoolMask schoolMask, SpellEntry const * spellProto)
{
    ...
    int32 levelChance = pVictim->GetTypeId() == TYPEID_PLAYER ? 7 : 11;
    int32 thisLevel = getLevelForTarget(pVictim);
    if (GetTypeId() == TYPEID_UNIT && ToCreature()->isTrigger())
        thisLevel = std::max<int32>(thisLevel, spellProto->spellLevel);
    int32 levelDiff = int32(pVictim->getLevelForTarget(this)) - thisLevel;
...
{
    // impossible de lancer un spell négatif sur les pilotes
    if (IsOnVehicle(pVictim) && !IsPositiveSpell(spell->Id))
        return SPELL_MISS_IMMUNE;

    // si on a le renvoi on renvoi la poigne de la mort :hap:
	if(pVictim->HasAura(23920) && spell->Id == 49576)
    ...
    // les sorts magiques de melee ne peuvent pas etre renvoyée (ex :  charge, interception, ...)
	...
	// le trait du chaos ne peux pas rater et 
	// si la victime a bubulle ou IB il est insensible sinon sa touche
	if(spell->Id == 50796 || spell->Id == 59170 || spell->Id == 59171 || spell->Id == 59172)
	{
		// bubulle
		if(pVictim->HasAura(642))
			return SPELL_MISS_IMMUNE;
		// iceblock
		if(pVictim->HasAura(45438))
			return SPELL_MISS_IMMUNE;
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i, AuraRemoveMode removeMode)
{
   ...
   // ASSERT(aurApp->GetTarget() == this);  
    ...
    if(caster && caster == this && aura->GetId() == 15473)
        caster->ToPlayer()->RemoveSpellCooldown(15473, true);
...
void Unit::RemoveAurasDueToSpellByDispel(uint32 spellId, uint64 casterGUID, Unit *dispeller, uint8 chargesRemoved)
{
    for (AuraMap::iterator iter = m_ownedAuras.lower_bound(spellId); iter != m_ownedAuras.upper_bound(spellId);)
    {
...
bool Unit::HasAura(uint32 spellId, uint64 casterGUID, uint64 itemCasterGUID, uint8 reqEffMask) const
{
    ...
    if (GetAuraApplication(spellId, casterGUID, itemCasterGUID, reqEffMask) || GetOwnedAura(spellId, casterGUID, itemCasterGUID, reqEffMask))
        return true;
    return false;
}
...
int32 Unit::GetTotalAuraModifierByMiscValue(AuraType auratype, int32 misc_value, uint32 spellId) const
{
    int32 modifier = 0;
    ...
    for (AuraEffectList::const_iterator i = mTotalAuraList.begin(); i != mTotalAuraList.end(); ++i)
    {
        if ((*i)->GetMiscValue() == misc_value)
            if (!(spellId == 48156 && (*i)->GetId() == 58943))
                modifier += (*i)->GetAmount();
    }
    return modifier;
}
    ...
    switch(dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (dummySpell->SpellFamilyFlags[1] & 0x00000010)
            {
                ...
                if (procSpell && procSpell->Id == dummy_spell)
                    return false;
                ...
                if (triggeredByAura->GetAmount() <= int32(damage) || GetHealth() <= damage)
                {
                    ...
                    if (Unit* caster = GetUnit(*this, casterGuid))
                        caster->CastSpell(this, dummy_spell, true, castItem);
...
int32 Unit::DealHeal(Unit *pVictim, uint32 addhealth)
{
    ...
    if (unit->HasAura(69172))
    {
        if (Creature* tyrannus = unit->FindNearestCreature(36658, 25.0f))
        {
            if(tyrannus)
            {
                int32 heal = addhealth ;
                tyrannus->CastCustomSpell(tyrannus, 69190, &heal, 0, 0, true);
            }
        }
    }	
...
Unit* Unit::SelectMagnetTarget(Unit *victim, SpellEntry const *spellInfo)
{
    ...
    //Do not redirect AoEs
    if(spellInfo)
        for (int8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            if (spellInfo->EffectImplicitTargetA[i] == TARGET_UNIT_AREA_ENEMY_SRC ||
                spellInfo->EffectImplicitTargetA[i] == TARGET_UNIT_AREA_ENEMY_DST ||
                spellInfo->EffectImplicitTargetA[i] == TARGET_SRC_CASTER ||
                spellInfo->EffectImplicitTargetA[i] == TARGET_DEST_DYNOBJ_ENEMY)
                return victim;
    ...
    switch(spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_HUNTER:
            ...
            if(spellProto->SpellFamilyFlags[1] & 0x1)
            {
                ...
                if (AuraEffect * aurEff = GetAuraEffect(56826, 0))
...
bool Unit::IsImmunedToSpell(SpellEntry const* spellInfo)
{
    ...
    // on peut iceblock avec Forbearance
    if(HasAura(25771) && spellInfo->Id == 45438) 
        return false;
...
bool Unit::isAlwaysVisibleFor(WorldObject const* seer) const
{
    ...
    // Les pets sont visibles par tout le groupe
...
{
    //don't mod permament auras duration + envenom
        ...
        for (uint8 effIndex = 0; effIndex<MAX_SPELL_EFFECTS; ++effIndex)
        {
            ...
            switch(triggeredByAura->GetAuraType())
            {
                ...
                case SPELL_AURA_PROC_TRIGGER_DAMAGE:
                {
                    ...
                    CalculateSpellDamageTaken(&damageInfo, damage, spellInfo);
                    DealDamageMods(damageInfo.target,damageInfo.damage,&damageInfo.absorb);
                    damageInfo.damage = DealSpellDamage(&damageInfo, true);
                    SendSpellNonMeleeDamageLog(&damageInfo);
                    takeCharges = true;
                    break;
                }
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const * aurApp)
{
    ...
    // Odyssee ASSERT((type == CHARM_TYPE_VEHICLE) == IsVehicle());
...
uint32 Unit::GetCombatRatingDamageReduction(CombatRating cr, float rate, float cap, uint32 damage, bool crit) const
{
    float pourcent = 1.1f;
    if(crit)
        pourcent = 1.2f;

    float percent = std::min(GetCombatRatingReduction(cr) * rate, cap) * pourcent;
