        ...
        if(m_PlayersInWar[plr->GetTeamId()].find(plr->GetGUID())!=m_PlayersInWar[plr->GetTeamId()].end())
        {
            ...
            if(Group* group = GetGroupPlayer(plr->GetGUID(), plr->GetTeamId())) // remove from raid group if player is member
            {
                ...
                if(!group->RemoveMember(plr->GetGUID()))
                    m_Groups[plr->GetTeamId()].erase(groupGuid);
            }
...
void Battlefield::InvitePlayerToQueue(Player* plr)
{
    if(m_PlayersInQueue[plr->GetTeamId()].count(plr->GetGUID()))
        return;
    if (m_PlayersInQueue[plr->GetTeamId()].size() <= m_MinPlayer || m_PlayersInQueue[plr->GetTeamId()].size() >= m_MinPlayer)
    {
...
uint32 Battlefield::getAlliancePlayersInWarCount() {
    return m_PlayersInWar[TEAM_ALLIANCE].size();
}

uint32 Battlefield::getHordePlayersInWarCount() {
    return m_PlayersInWar[TEAM_HORDE].size();
}
