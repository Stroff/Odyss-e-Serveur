...
void BattlefieldMgr::HandlePlayerEnterZone(Player *plr, uint32 zoneid)
{
    ...
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Player %u entered outdoorpvp id %u", plr->GetGUIDLow(), itr->second->GetTypeId());
}
...
void BattlefieldMgr::HandlePlayerLeaveZone(Player *plr, uint32 zoneid)
{
    ...
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Player %u left outdoorpvp id %u",plr->GetGUIDLow(), itr->second->GetTypeId());
}
