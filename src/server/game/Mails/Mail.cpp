...
void WorldSession::SendExternalMails()
{
    ...
    QueryResult result = CharacterDatabase.Query("SELECT e.id, e.receiver, e.subject, e.message, e.money, i.item FROM mail_external e LEFT JOIN mail_external_items i ON e.id = i.mail_id WHERE e.sent = 0 ORDER BY e.id;");
    if(!result)
    {
    ...
    else
    {
        ...
        do
        {
            ...
            uint32 itemCount = 1; //fields[6].GetUInt32();
