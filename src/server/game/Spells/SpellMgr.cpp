...
bool SpellMgr::_isPositiveEffect(uint32 spellId, uint32 effIndex, bool deep) const
{
    ...
    switch (spellproto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_SHAMAN:
            if (spellId == 30708)
                return false;
            // totem de lien de terre amélioré
			if (spellId == 59566)
				return true;
            break;
        case SPELLFAMILY_WARRIOR:
    ...
    switch (spellproto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if  (spellproto->SpellIconID == 1548)
                return DIMINISHING_NONE; // pour ne pas aller sur le même DR que le sheep
            // Frostbite
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            else if (spellproto->SpellFamilyFlags[0] & 0x100000)
                return DIMINISHING_LIMITONLY;
            // Turn Evil
...
bool CanSpellPierceImmuneAura(SpellEntry const * pierceSpell, SpellEntry const * aura)
{
    ...
    if ((pierceSpell->AttributesEx & SPELL_ATTR1_UNAFFECTED_BY_SCHOOL_IMMUNE)
        // ...but not these (Divine shield for example) ou le cyclone lui même...
        && !(aura && (aura->Mechanic == MECHANIC_IMMUNE_SHIELD || aura->Mechanic == MECHANIC_INVULNERABILITY || aura->Id == 33786)))
        return true;
...
void SpellMgr::LoadSpellCustomAttr()
{
    ...
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        case 19578:
        case 20895:
            spellInfo->excludeCasterAuraSpell = spellInfo->Id ;
