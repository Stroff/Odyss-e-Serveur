    ...
    else if (pushType)
    {
        ...
        if (!unitList.empty())
        {
            ...
            else
            {
                switch (m_spellInfo->Id)
                {
                    case 27285: // Seed of Corruption proc spell rank 1
                    case 47833: // Seed of Corruption proc spell rank 2
                    case 47834: // Seed of Corruption proc spell rank 3
...
void Spell::cast(bool skipCheck)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            if( m_spellInfo->Id == 66588 ) // Sus au kraken hack
                if( Unit *unit = m_targets.getUnitTarget() )
                    if ( unit->GetEntry() == 34925 || unit->GetEntry() == 35092 )
                      m_caster->DealDamage( unit, urand(97000,104000) );				
            break;
        }
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (target)
    {
        ...
        if (m_spellInfo->Id==51325 || m_spellInfo->Id==51326 || m_spellInfo->Id==51327 || m_spellInfo->Id==51328)
        {
            if (m_caster->ToPlayer())
                if (m_caster->ToPlayer()->InArena())
