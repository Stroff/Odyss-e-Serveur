...
void Spell::EffectInstaKill(SpellEffIndex /*effIndex*/)
{
    ...
    // Petrov's clusters bombs usable on only the quest mobs
    if (m_spellInfo->Id == 42366)
    {
...
void Spell::SpellDamageSchoolDmg(SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->isAlive())
    {
        if (unitTarget->HasAura(64373)) //Armistice
            return;	
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                switch(m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    case 62311: case 64596: // Cosmic Sma	sh
                    {
                        float x = unitTarget->GetPositionX(); 
                        float y = unitTarget->GetPositionY();
                        float distance = unitTarget->GetDistance2d(x,y);
            ...
            case SPELLFAMILY_DRUID:
            {
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER && (m_spellInfo->SpellFamilyFlags[0] & 0x000800000) && m_spellInfo->SpellVisual[0] == 6587)
                {
                    ...
                    damage += int32(CalculatePctN(m_caster->ToPlayer()->GetComboPoints() * ap, 5)); //nerf dégats du feral (normalement 7)
                }
                // Wrath
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                else if (m_spellInfo->SpellFamilyFlags[EFFECT_1] & 0x100000)
                {
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 12868:
                {
                    ...
                    AuraEffect* deepWound = unitTarget->GetAuraEffect(12721, 0, m_originalCasterGUID);
                    if (deepWound)
                    {
                ...
                case 32146 :                                // Liquid fire
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    if(Creature* villager = m_caster->FindNearestCreature(18240, 2.0f, false))
                    {
                        ((Player*)m_caster)->KilledMonsterCredit(18240, 0) ;
                        villager->DisappearAndDie() ;
                    }
                    return;					
                ...
                case 59728:                                // Retrieve Data Credit
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER)                        
                        return;
                    if (m_caster->GetTypeId() != TYPEID_UNIT)
                        return;
                    ((Player*)unitTarget)->KilledMonsterCredit(29752, 0);
                    ((Creature*)m_caster)->DisappearAndDie();
                    return;
                case 45980:                                    // Re-Cursive Transmatter Injection
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 25814)
                        return;
                    if(unitTarget->isAlive())
                        return;
                    unitTarget->SummonCreature(25773, unitTarget->GetPositionX(), unitTarget->GetPositionY(), unitTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 5000);
                    ((Creature*)unitTarget)->RemoveCorpse();
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    ((Player*)m_caster)->KilledMonsterCredit(25773, 0);
                    return;
                case 46485:                                    // The Greatmother's Soulcatcher
                    ...
                    if(unitTarget->isAlive())
                        return;
                    unitTarget->SummonCreature(26096, unitTarget->GetPositionX(), unitTarget->GetPositionY(), unitTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 5000);
                    ((Creature*)unitTarget)->RemoveCorpse();
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    ((Player*)m_caster)->KilledMonsterCredit(26096, 0);
                    return;
                case 62990:                                // Chop Tree
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    if(Creature* oak = m_caster->FindNearestCreature(33308, 2.0f))
                    {
                        ItemPosCountVec dest;
                        uint8 msg = ((Player*)m_caster)->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, 45045, 1, false);
                        if (msg == EQUIP_ERR_OK)
                        {
                            ((Player*)m_caster)->StoreNewItem(dest, 45045, 1, true);
                            oak->DisappearAndDie();
                        }
                    }
                    return;
                case 47110:                                // Drakuru's Elixir
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    if(GameObject* brazier = m_caster->FindNearestGameObject(188287, 10.0f))
                        ((Player*)m_caster)->SummonCreature(26500, brazier->GetPositionX(), brazier->GetPositionY(), brazier->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 30000);
                    return;
                case 54577:                                // Throw U.D.E.D.
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 29402)
                        return;
                    switch(rand()%2)
                    {
                        case 0: 
                            unitTarget->CastSpell(unitTarget, 54625, true); 
                            break;
                        case 1: 
                            unitTarget->CastSpell(unitTarget, 57444, true);
                            break;
                    }
                    ((Creature*)unitTarget)->DisappearAndDie();
                    return;
                case 62272:        // Raelorasz Spark
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    m_caster->SummonCreature(26237, m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 30000);
                    ((Player*)m_caster)->KilledMonsterCredit(26237, 0);
                    return;
                case 47911:                                // EMP
                    if(!unitTarget)
                        return;
                    if (unitTarget->HasAura(47922))
                        unitTarget->RemoveAurasDueToSpell(47922);
                    return;

                case 6509:                                // Gore Bladder
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 29392 || unitTarget->isAlive())                        
                        return;
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    ((Player*)m_caster)->KilledMonsterCredit(29391, 0);
                    ((Creature*)unitTarget)->DisappearAndDie();
                    return ;
                case 51330:                                // Shoot RJR
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 28054)
                        return;
                    switch(urand(0,5))
                    {
                        case 0:
                            m_caster->CastSpell(unitTarget, 51366, true);
                            break;
                        case 1:
                            m_caster->CastSpell(unitTarget, 51331, true);
                            break;
                        default:
                            m_caster->CastSpell(unitTarget, 51332, true);
                            break;
                    }
                    return;
                case 51366:                                // Miss Apple, Hit Bird
                    if(!unitTarget)
                        return;
                    if(Creature* bird = unitTarget->FindNearestCreature(28093, 30.0f))
                        m_caster->Kill(bird);
                    return;
                case 51331:                                // Hit Apple
                    if(!unitTarget)
                        return;
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    if(Creature* apple = unitTarget->FindNearestCreature(28053, 5.0f))
                    {
                        ((Player*)m_caster)->KilledMonsterCredit(28053, 0);
                        apple->DisappearAndDie();
                    }
                    return;
                case 51332:                                // Miss Apple
                    return;
                case 4338:
                case 59643:
                    if (m_caster->GetTypeId() != TYPEID_UNIT)
                        return;
                    if(!m_caster->GetOwner() && m_caster->GetOwner()->GetTypeId() != TYPEID_PLAYER)
                        return;
                    ((Player*)m_caster->GetOwner())->KilledMonsterCredit(31766, 0);
                    return;
                case 51895:                                // Whisker of Har'koa
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 28404 || unitTarget->isAlive())                        
                        return;
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    unitTarget->SummonCreature(28526, unitTarget->GetPositionX(), unitTarget->GetPositionY(), unitTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 10000);
                    ((Player*)m_caster)->KilledMonsterCredit(28526, 0);
                    ((Creature*)unitTarget)->DisappearAndDie();
                    return;
                case 46606:                                // Tillinghast's Plague Canister Dummy
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 23777 || !unitTarget->isAlive())                        
                        return;
                    unitTarget->SummonCreature(24160, unitTarget->GetPositionX(), unitTarget->GetPositionY(), unitTarget->GetPositionZ(), 0,  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000);
                    unitTarget->DealDamage(unitTarget, unitTarget->GetHealth());
                    return;
                case 47305:                                // Potent Explosive Charge
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 26293 || !unitTarget->isAlive())                        
                        return;
                    unitTarget->CastSpell(unitTarget, 47311, true);
                    unitTarget->CastSpell(unitTarget, 47309, true);
                    unitTarget->CastSpell(unitTarget, 47315, true);
                    ((Creature*)m_caster)->DisappearAndDie();
                    return;
                case 48297:
                case 49285:
                    ...
                    ((Creature*)m_caster)->DisappearAndDie();
                    return;
                case 47627:                                // Wildlife Innoculation
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || !unitTarget->isAlive() || unitTarget->HasAura(47675))        
                        return;
                    if(unitTarget->GetEntry() == 26615)
                        return;
                    ((Player*)m_caster)->KilledMonsterCredit(26895, 0) ;
                    if(unitTarget->GetEntry() == 26482)
                        return;
                    ((Player*)m_caster)->KilledMonsterCredit(26882, 0) ;
                    unitTarget->AddAura(47675, unitTarget);
                    return ;
                case 52090:                                // Ahunae's Knife
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->isAlive())                        
                        return;
                    if(unitTarget->GetEntry() != 28600 && unitTarget->GetEntry() != 28465)
                        return;
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    ((Player*)m_caster)->KilledMonsterCredit(28622, 0);
                    ((Creature*)unitTarget)->DisappearAndDie();
                    return ;
                case 54015:                                // Set Oracle Faction Honored
                    ...
                    if (unitTarget->GetTypeId() != TYPEID_PLAYER)
                        return;
                    ((Player*)unitTarget)->SetReputation(1105, 9001);
                    ((Player*)unitTarget)->SetReputation(1104, -57000);
                    return;
                case 53475:                                // Set Oracle Faction Friendly
                    ...
                    if (unitTarget->GetTypeId() != TYPEID_PLAYER)
                        return;
                    ((Player*)unitTarget)->SetReputation(1105, 3001);
                    return;
                case 53487:                                // Set Wolvar Faction Honored
                    ...
                    if (unitTarget->GetTypeId() != TYPEID_PLAYER)
                        return;
                    ((Player*)unitTarget)->SetReputation(1104, 9001);
                    ((Player*)unitTarget)->SetReputation(1105, -57000);
                    return;
                case 51659:                                // Apply Harness
                    if (!unitTarget)
                        return;
                ...
                case 49266:                                // Dangle Wild Carrot
                    if (!unitTarget)
                        return;
                    m_caster->CastSpell(unitTarget, 49282, true);
                    return;
                case 45524:
                {
                ...
                case 49587:                                    // That Which can give life can also ...
                {
                    if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT)
                        return;
                    if(unitTarget->GetEntry() == 26841)
                        ((Creature*)unitTarget)->UpdateEntry(27821);
                    if(unitTarget->GetEntry() == 27808)
                        ((Creature*)unitTarget)->UpdateEntry(27809);
                    if(unitTarget->GetEntry() == 27122)
                        ((Creature*)unitTarget)->UpdateEntry(27807);
                    return ;
                }
                case 46770:
                    if(!unitTarget || (unitTarget->GetTypeId() != TYPEID_UNIT) || unitTarget->isAlive() || unitTarget->HasAura(47972))
                        return;
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    if(unitTarget->GetEntry() == 26616)
                    {
                        ((Player*)m_caster)->KilledMonsterCredit(27111, 0);
                        m_caster->AddAura(47972, unitTarget);
                    }
                    if(unitTarget->GetEntry() == 26643)
                    {
                        ((Player*)m_caster)->KilledMonsterCredit(27112, 0);
                        m_caster->AddAura(47972, unitTarget);
                    }
                    return;				
                ...
                case 51582:                                 //Rocket Boots Engaged (Rocket Boots Xtreme and Rocket Boots Xtreme Lite)
                {
                    ...
                    m_caster->ToPlayer()->RemoveSpellCooldown(30452, true);
                    m_caster->CastSpell(m_caster, 30452, true, NULL);
                  //  for(int i= 0; m_caster->IsFlying() && i < 15000; i++)         Ceci serai pour mettre l'ignition (tourbilol tant que le player est en l'air
                  //     m_caster->CastSpell(m_caster, 39850, true, NULL);          et le i pour sécuriser au cas ou un mec l'use en flyhack ou hauteur gros importate
                  //                                                                Elyotna m'a dit non car cela freezerai le serveur :D
                    m_caster->ToPlayer()->AddSpellCooldown(30452, m_CastItem->GetEntry(), time(NULL) +300);
                    return;
                }
                        ...
                        for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
                            if (ihit->targetGUID != m_caster->GetGUID())
                                if (Player *target = ObjectAccessor::FindPlayer(ihit->targetGUID))
                                    if (target->HasAura(m_triggeredByAuraSpell->Id))
                                        break ;
                        m_caster->CastSpell(unitTarget,71341,true) ;
                    }
                    break ;
                case 71341:        // Pact of the darkfallen
                    for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
                        if (ihit->targetGUID != m_caster->GetGUID())
                             if (Player *target = ObjectAccessor::FindPlayer(ihit->targetGUID))
                                if (!target->HasAura(71340))
                                    m_caster->DealDamage(target, 3500, 0, SPELL_DIRECT_DAMAGE, SPELL_SCHOOL_MASK_SHADOW, m_spellInfo);
                     break ;
                case 8283:        // Snufflenose Command
                    if (!unitTarget)
                        return ;
                    if (unitTarget->GetEntry() != 4781)
                        return ;
                    if(rand()%2 == 0)
                        unitTarget->SummonGameObject(20920, unitTarget->GetPositionX(), unitTarget->GetPositionY(), unitTarget->GetPositionZ(), 0, 0, 0, 0, 0, 0);
                    break ;
                case 67322:
                    if (!unitTarget)
                        return ;
                    if (!unitTarget->HasAura(66193))
                        unitTarget->CastSpell(unitTarget, 68394, true);
                    break ;
                case 72040:        // Conjure empowered flame
                    if (!m_caster)
                        return;
                    m_caster->CastSpell(m_caster, 72041, true);
                    break;
                case 71718:        // Conjure empowered flame
                    if (!m_caster)
                        return;
                    m_caster->CastSpell(m_caster, 71719, true);
                    break;
                case 46222:        // Animal blood
                    ...
                    if (!unitTarget->IsInWater())
                        return;
                    unitTarget->SummonGameObject(194479, m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), 0, 0, 0, 0, 0, 30000);
                    unitTarget->RemoveAurasDueToSpell(46221);
                    break;
                case 13006:     // Shrink Ray
                    ...
                    switch(rand()%8)
                    {
						...
						case 4:
						{
							...
							m_caster->GetPartyMemberInDist(members, 30.0f);
							for(std::list<Unit*>::iterator itr = members.begin(); itr != members.end(); ++itr)
							{
								if (*itr)
									m_caster->CastSpell((*itr), 13003, true);
							}
							break;
						}
						case 5:
						{
							...
							for(std::list<Unit*>::iterator itr = members.begin(); itr != members.end(); ++itr)
								if (*itr)
									m_caster->CastSpell((*itr), 13004, true);
							break;
						}
						case 6:
						{
							std::set<Unit*> const& attackers = m_caster->getAttackers();
							for (std::set<Unit*>::const_iterator itr = attackers.begin(); itr != attackers.end();++itr)
								if (*itr)
									m_caster->CastSpell((*itr), 13003, true);
							break;
						 }
				...
				break;
                case 51420: // Digging for treasure ping (not good creature)
                    if (!m_caster)
                        return;
                    if (Creature* treasure = m_caster->FindNearestCreature(28455, 5.0f, true))
                    {
                        switch(rand()%7)
                        {
                        case 0:
                            m_caster->CastSpell(unitTarget, 51402, true);
                            break;
                        case 1:
                            m_caster->CastSpell(unitTarget, 51398, true);
                            break;
                        case 2:
                            m_caster->CastSpell(unitTarget, 51443, true);
                            break;
                        case 3:
                            m_caster->CastSpell(unitTarget, 51400, true);
                            break;
                        case 4:
                            m_caster->CastSpell(unitTarget, 51401, true);
                            break;
                        case 5:
                            m_caster->CastSpell(unitTarget, 51397, true);
                            break;
                        case 6:
                            m_caster->CastSpell(unitTarget, 51441, true);
                            break;
                        }
                        treasure->ForcedDespawn();
                    }
                break;
                case 58984: // Racial elfe de la nuit
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            if (m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_DK_DEATH_STRIKE)
            {
                ...
                // Improved Death Strike (ne s'applique pas à la main gauche)
                ...
                // si on a 2 armes les effects sont reduits par 2 (plus de usebug spe givre :p)
                if (m_caster->ToPlayer() && m_caster->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND))
            		bp /= 2;
            ...
            case 49560: // Death Grip
				...
				else if (unitTarget->HasAuraType(SPELL_AURA_REFLECT_SPELLS)) //renvoi
				{
					float positionX = unitTarget->GetPositionX();
					float positionY = unitTarget->GetPositionY();
					float positionZ = unitTarget->GetPositionZ();
					m_caster->CastSpell(positionX, positionY, positionZ, damage, true);
				}
                ...
                // On stop cast lors de qu'on "tp" au lieu de cast pendant le mouvement
                unitTarget->CastStop();
                return;
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 50476:
        {
            m_caster->NearTeleportTo(968.66f, 1042.487f, m_caster->GetPositionZ(), 0) ;
            break ;
        }
        case 50495:
        {
            m_caster->NearTeleportTo(1163.48f, 1171.04f, m_caster->GetPositionZ(), 0) ;
            break ;
        }
        case 50496:
        {
            m_caster->NearTeleportTo(1120.85f, 1083.71f, 508.37f, 0) ;
            break ;
        }		
...
void Spell::EffectApplyAura(SpellEffIndex effIndex)
{
    ...
    /* esprit farouche */
    if(m_spellInfo->Id == 58876 && m_caster->GetOwner())
    {
        m_caster->GetOwner()->RemoveMovementImpairingAuras();
    }

    m_spellAura->_ApplyEffectForTargets(effIndex);
    switch(m_spellInfo->Id)
    {            
        case 61187: // spell sartha - phase by Nonox & un pti peu agma mais c'est nonox le mac
        {
            if(!unitTarget)
                return;
            unitTarget->RemoveAura(57620);
            unitTarget->RemoveAura(57874);
            unitTarget->SetPhaseMask(1, 1);
            break;
        }
    }	
}
    ...
    if (gameObjTarget)
    {
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_GOOBER:
                ...
                {
                    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "Goober ScriptStart id %u for GO %u",gameObjTarget->GetGOInfo()->goober.eventId,gameObjTarget->GetDBTableGUIDLow());
    ...
    switch(m_spellInfo->Id)
    {
		...
		case 30706:
			if(m_caster->HasAura(63280))
				m_caster->CastSpell(m_caster, 63283, true);
			break ;
		default:
			break ;
    }	
...
void Spell::EffectDispel(SpellEffIndex effIndex)
{
    ...
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if (unitTarget->HasAura(50536) && aura->GetSpellProto()->Dispel == DISPEL_DISEASE) // Unholy blight talent work.
            continue;			
        ...
        if ((1<<aura->GetSpellProto()->Dispel) & dispelMask)
        {
            if (aura->GetSpellProto()->Dispel == DISPEL_MAGIC)
            {
                ...
                // permet a tous les dispels magiques d'entrer en combats si sa target n'est pas un amis :D
                if(m_caster && unitTarget) // pour sécuriser
                {
...
void Spell::SpellDamageWeaponDmg(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            {
                AddPctN(totalDamagePercentMod, SpellMgr::CalculateSpellEffectAmount(m_spellInfo, EFFECT_2) * unitTarget->GetDiseasesByCaster(m_caster->GetGUID()) *0.9f);
                break;
            }
            break;
        }
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 61122:
                {
                m_caster->SummonCreature(29579, m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 120000) ;
                break ;
                }
                case 43972:    // Mixing Blood
                {
					case 0 : m_caster->CastSpell(m_caster, 43376, true) ; break ;
					case 1 : m_caster->CastSpell(m_caster, 43970, true) ; break ;
					case 2 : m_caster->CastSpell(m_caster, 43378, true) ; break ;
					case 3 : m_caster->CastSpell(m_caster, 43377, true) ; break ;
                }
                case 47703:                                // Unholy Union
					if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 26926 || !unitTarget->isAlive())                        
						return ;
					m_caster->CastSpell(m_caster, 50254, true) ;
					return ;
                case 50252:                                // Blood Draw
					if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 26411 || !unitTarget->isAlive())                        
						return ;
					m_caster->CastSpell(m_caster, 50250, true) ;
					return ;
                case 47724:                                // Frost Draw
					if(!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || unitTarget->GetEntry() != 26283 || !unitTarget->isAlive())                        
						return ;
                ...
                case 47097:                        // Surge Needle Teleporter
                    if (m_caster->GetTypeId() != TYPEID_PLAYER)
                        return;
                    if (m_caster->GetAreaId() != 4157)
                        return ;
                    if (m_caster->GetPositionZ() >= 191)
                    {
                        m_caster->ToPlayer()->TeleportTo(m_caster->ToPlayer()->m_recallMap, m_caster->ToPlayer()->m_recallX, m_caster->ToPlayer()->m_recallY, m_caster->ToPlayer()->m_recallZ, 0, 0) ;
                    }
                    else
                    {
                        m_caster->ToPlayer()->SaveRecallPosition() ;
                        m_caster->ToPlayer()->TeleportTo(m_caster->GetMapId(), 3338.57f, 2518.44f, 197.32f, 0, 0) ;
                    }
                    return ;
                case 45759:                        // Warsong Orc Disguise
                    ...
                    if (m_caster->getGender() == GENDER_MALE)
                        m_caster->CastSpell(m_caster, 45760, true) ;
                    if (m_caster->getGender() == GENDER_MALE)
                        m_caster->CastSpell(m_caster, 45762, true) ;
                    return ;
                case 66741:
                    if(m_caster->GetZoneId() != 210 || m_caster->GetAreaId() != 210)
                        return ;
                    switch(urand(1,4))
                    {
                    case 1 :
                        m_caster->CastSpell(m_caster, 66740, true) ;
                        break ;
                    case 2 :
                        m_caster->CastSpell(m_caster, 66739, true) ;
                        break ;
                    case 3 :
                        m_caster->CastSpell(m_caster, 66738, true) ;
                        break ;
                    case 4 :
                        break ;
                    }
                ...
                case 62575:        // Shield Breaker
                {
                    if(!unitTarget)
                        return ;
                    if (unitTarget->HasAura(66482))
                        unitTarget->GetAura(66482)->SetStackAmount(unitTarget->GetAura(66482)->GetStackAmount()-1);
                    break ;
                }
                case 68282:        // Charge
                {
                    ...
                    if (unitTarget->HasAura(66482))
                        unitTarget->GetAura(66482)->SetStackAmount(unitTarget->GetAura(66482)->GetStackAmount()-1);
                    m_caster->GetMotionMaster()->MoveCharge(unitTarget->GetPositionX(),unitTarget->GetPositionY(),unitTarget->GetPositionZ());
                    m_caster->DealDamage(unitTarget, 20000, 0, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, m_spellInfo) ;
                    break ;
                }				
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
    ...
    if (target->GetAreaId() != 4570 && caster->GetAreaId() != 4570) //Le cercle des volontés
    {
...
void Spell::EffectActivateObject(SpellEffIndex effIndex)
{
    ...
    {
		case 66512:
			m_caster->CastSpell(m_caster, 66510, true);
    }		
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const *properties)
{
    ...
    switch (m_spellInfo->Id)
    {
        case 1122: // Inferno
            amount = 1;
            break;
        case 48739: // Sauveteur aileron-d'hiver
        case 67761:
            amount = 1;
