...
void AuraEffect::PeriodicTick(AuraApplication * aurApp, Unit * caster) const
{
    ...
    if (target->HasAura(64373)) //Armistice
    {
...
// crash Odyssée            sLog->outDetail("PeriodicTick: %u (TypeId: %u) attacked %u (TypeId: %u) for %u dmg inflicted by %u abs is %u",
//                GUID_LOPART(GetCasterGUID()), GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(), target->GetTypeId(), damage, GetId(),absorb);
        ...
        case SPELL_AURA_PERIODIC_LEECH:
        {
            ...
            if (GetSpellProto()->Effect[GetEffIndex()] == SPELL_EFFECT_PERSISTENT_AREA_AURA &&
                caster->isAlive() && target->GetTypeId() == TYPEID_PLAYER && caster->GetTypeId() == TYPEID_PLAYER)
                caster->SetPvP(true);
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            ...
            else
            {
                ...
                // buff Egide démoniaque (+1% a grangarmure)
                if(m_spellProto->SpellFamilyName == SPELLFAMILY_WARLOCK)
                    if(caster->HasAura(30144) && caster->HasAura(28176))
                        damage += int32(float(caster->GetMaxHealth()) / 100.0f);
...
void AuraEffect::PeriodicDummyTick(Unit * target, Unit * caster) const
{
    switch (GetSpellProto()->SpellFamilyName)
    {
        ...
        switch (GetId())
        {
            ...
            case 68646:
            {
                int32 damage = (GetAmount() * target->GetHealth()) / 100;
                if (damage < 250)
                    damage = 250;
        ...
        switch (auraSpellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                switch (auraId)
                {
                    ...
                    // Time bomb
                    case 51121: triggerSpellId = 51132; break;
                    case 59376: triggerSpellId = 51132; break;					
                }
                break;
            }
            case SPELLFAMILY_MAGE:
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            switch(GetMiscValue())
            {
                case FORM_CAT:
                    ...
                    // Feral Swiftness
                    if (target->HasAura(24866))
                        target->CastSpell(target, 24864, true);
                    if (target->HasAura(17002))
                        target->CastSpell(target, 24867, true);					
                break;
                case FORM_DIREBEAR:
                case FORM_BEAR:
...
void AuraEffect::HandleModInvisibility(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        if (GetId() == 45614)    // Shroud of the Scourge
            if(target->GetZoneId() != 3537 || target->GetAreaId() != 4125)
                return ;
...
void AuraEffect::HandleModMechanicImmunity(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    switch (GetId())
    {
        ...
        case 59752: // Every Man for Himself
            ...
            break;
        case 63230: // Dispersion le seul buff avec effet de trinket sur 6 sec...
            mechanic = IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK;
    ...
    if (mode & AURA_EFFECT_HANDLE_REAL)
    {
        ...
        if (apply)
        {
            ...
            switch(GetId())
            {
                ...
                case 42644:
                    if(caster && caster->IsInWater() && caster->GetZoneId() == 15 && caster->GetAreaId() == 517)
                        if(Creature* temp = caster->SummonCreature(23928, caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
                            temp->SetInCombatWith(caster);
                    break ;
                case 67604:    // Powering Up
                    if(!target)
                        return ;
                    if(Aura* poweringUp = target->GetAura(67604))
                    {
                        if(poweringUp->GetStackAmount() == 99)
                        {
                            if(target->HasAura(65684))
                                target->CastSpell(target, 67215, true);
                            if(target->HasAura(65686))
                                target->CastSpell(target, 67218, true);
                            target->RemoveAurasDueToSpell(67604);
                        }
                    }
                    break ;
                case 57934:    // Tricks of the trade
                    ...
                    if (target == caster)
                        return;
                    caster->CastSpell(target, 57933, true);
                    caster->CastSpell(target, 59628, true);
                    break;
                case 49592:    // Temporal Shift
                    if(!target)
                        return;
                    target->CastSpell(target, 49836, true);
                    break;
                case 49838:
                    if(!target)
                        return;
                    target->CastSpell(target, 49836, true);
                    if (Aura* aura = target->GetAura(49836))
                ...
                case SPELLFAMILY_GENERIC:
                    switch(GetId())
                    {
                        ...
                        case 68839: // Corrupt Soul
                            ...
                            target->RemoveAura(58768);
                            target->RemoveAurasDueToSpell(58768);							
                            break;
                    }
                    break;
                case SPELLFAMILY_MAGE:
    ...
    switch(m_spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            switch(GetId())
            {
                ...
                // Inactive
                case 43681:
                    if (target && target->GetTypeId() == TYPEID_PLAYER && target->ToPlayer()->InBattleground())
                        target->ToPlayer()->LeaveBattleground();
                    break;
                case 62061: // Festive Holiday Mount
                    if (target->HasAuraType(SPELL_AURA_MOUNTED))
