...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "AuraApplication::_HandleEffect: %u, apply: %u: amount: %d", aurEff->GetAuraType(), apply, aurEff->GetAmount());
            ...
            case SPELLFAMILY_PRIEST:
                ...
                if (GetSpellProto()->SpellFamilyFlags[0] & 0x02000000 && GetEffect(0))
                {
                    ...
                    if (AuraEffect const * aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 3790, 1))
                    {
                        ...
                        // heal de 15% des dégats lors de l'application de la peste devo
            ...
            case SPELLFAMILY_GENERIC:
                switch(GetId())
                {
                    ...
                    if (target->GetPower(POWER_MANA))
                        target->SetPower(POWER_MANA, target->GetPower(POWER_MANA) + 1500);
                         break;						
