...
ChatCommand * ChatHandler::getCommandTable()
{
    ...
    static ChatCommand commandTable[] =
    {
        ...
        { "bump",           SEC_GAMEMASTER,     false, OldHandler<&ChatHandler::HandleBumpCommand>,                "", NULL },
        { "tpjoug",         SEC_PLAYER,         false, OldHandler<&ChatHandler::HandleTPJougCommand>,              "", NULL },
        ...
        if ((table[i].Handler)(this, strlen(table[i].Name) != 0 ? text : oldtext))
        {
            if (table[i].SecurityLevel > SEC_PLAYER)
            {
                ...
                if (m_session)
                {
                    ...
                    if(p->GetSelectedPlayer())
                    {
                        sLog->outCommand(m_session->GetAccountId(),"Command: %s [Player: %s (Account: %u) X: %f Y: %f Z: %f Map: %u Selected: %s (GUID: %u NAME: %s)]",
                            fullcmd.c_str(),p->GetName(),m_session->GetAccountId(),p->GetPositionX(),p->GetPositionY(),p->GetPositionZ(),p->GetMapId(),
                            GetLogNameForGuid(sel_guid),GUID_LOPART(sel_guid), p->GetSelectedPlayer()->GetName());
                    }
                    else
                    {
                    ...
                    std::string c_fullcmd( fullcmd );
                    CharacterDatabase.escape_string(c_fullcmd);
                    std::vector<std::string> eCmd = explode(" ", c_fullcmd);
                    char pQuery[2000];
                    int length = 0;

                    sprintf( pQuery, "INSERT gmlogs VALUES ( '0',NOW(),'%s','%u','%s','%f','%f','%f','%u','%s','%u','%s'",
                        p->GetName(),
                        ...
                        p->GetPositionZ(),
                        p->GetMapId(),
                        GetLogNameForGuid(sel_guid),
                        GUID_LOPART(sel_guid),
                        eCmd[0].c_str() );
                       
                    length += eCmd[0].size() + 1;
                    std::string sQuery( pQuery );
                    for( uint16 i = 1; i < 4; ++i )
                        if( i < eCmd.size() )
                        {
                            sQuery += ", '" + eCmd[i] + "'";
                            length += eCmd[i].size() + 1;
                        }
                        else sQuery += ", ''";

                    if( c_fullcmd.size() > length )
                        sQuery += ", '" + c_fullcmd.substr(length) + "')";
                    else sQuery += ", '')";

                    CharacterDatabase.PExecute( sQuery.c_str() );
                }
            }
        }
