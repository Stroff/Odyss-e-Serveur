...
class ChatHandler
{
    ...
    protected:
        ...
        bool HandleTPJougCommand( const char * );
        ...
        bool HandleWintergraspWinCommand(const char *args);		

        bool HandleBumpCommand(const char* args);        

        bool HandleRecupCommand(const char* args);
        bool HandleRecupLearnCommand(uint32 arg);
        bool HandleRecupSetSkillCommand(uint32 skill, uint32 level);
        bool HandleRecupAddItemCommand(uint32 itemId, int32 count = 1);
