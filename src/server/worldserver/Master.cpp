...
int Master::Run()
{
    ...
    sLog->outString( "  ____  _______     _______ _____ ______ ______ ");
    sLog->outString( " / __ \\|  __ \\ \\   / / ____/ ____|  ____|  ____|");
    sLog->outString( "| |  | | |  | \\ \\_/ / (___| (___ | |__  | |__   ");
    sLog->outString( "| |  | | |  | |\\   / \\___ \\\\___ \\|  __| |  __|  ");
    sLog->outString( "| |__| | |__| | | |  ____) |___) | |____| |____ ");
    sLog->outString( " \\____/|_____/  |_| |_____/_____/|______|______|");
    sLog->outString( "http://odyssee-serveur.com     ");
...
bool Master::_StartDB()
{
    ...
    ///- Get recup database info from configuration file
    dbstring = sConfig->GetStringDefault("RecupDatabaseInfo", "");
    if (dbstring.empty())
    {
        sLog->outError("Recup database not specified in configuration file");
        return false;
    }

    async_threads = sConfig->GetIntDefault("RecupDatabase.WorkerThreads", 1);
    if (async_threads < 1 || async_threads > 32)
    {
        sLog->outError("Recup database: invalid number of worker threads specified. "
            "Please pick a value between 1 and 32.");
        return false;
    }

    synch_threads = sConfig->GetIntDefault("RecupDatabase.SynchThreads", 1);

    ///- Initialise the recup database
    if (!RecupDatabase.Open(dbstring, async_threads, synch_threads))
    {
        sLog->outError("Cannot connect to recup database %s", dbstring.c_str());
        return false;
    }
