...
#define QUETE_PVP            89463
#define EVENT_MORPH            201
...
{
    if (CAST_PLR(killer)->GetQuestStatus(QUETE_PVP) == QUEST_STATUS_INCOMPLETE)
        {CAST_PLR(killer)->KilledMonsterCredit(QUETE_PVP, 0);}
}
...
void OnCreatureKill(Player * pPlayer, Creature * pCreature)
{
    ...
    if (IsEventActive(EVENT_MORPH))
        {
            if (pCreature->GetMaxHealth() < 15000)
            {
            pPlayer->SetDisplayId(pCreature->GetDisplayId());
            }
        }
