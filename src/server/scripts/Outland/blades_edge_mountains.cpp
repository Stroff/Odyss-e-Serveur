...
enum SpiritCalling
{
    NPC_SPIRIT_CALLING_TOTEM        = 22318,
    QUEST_SPIRIT_CALLING            = 10853,
    ITEM_SPIRIT_CALLING_TOTEM        = 31663
};

#define GOSSIP_HELLO_TOTEM "I want more totems."

class mob_lesser_nether_drake : public CreatureScript
{
public:
    mob_lesser_nether_drake() : CreatureScript("mob_lesser_nether_drake") { }

    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new mob_lesser_nether_drakeAI(pCreature);
    }
struct mob_lesser_nether_drakeAI : public ScriptedAI
{
    mob_lesser_nether_drakeAI(Creature *c) : ScriptedAI(c) {}

    void Reset() 
    { 
        intangiblePresenceTimer = urand(8000, 12000) ;
    }
    ...
    {
        if (me->FindNearestCreature(NPC_SPIRIT_CALLING_TOTEM, 15.0f))
            me->SetLootMode(2) ;
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (intangiblePresenceTimer <= uiDiff)
        {
            DoCast(SPELL_INTANGIBLE_PRESENCE) ;
            intangiblePresenceTimer = urand(25000, 28000) ;
        } else intangiblePresenceTimer -= uiDiff ;
...
class npc_spiritcaller_dohgar : public CreatureScript
{
public:
    npc_spiritcaller_dohgar() : CreatureScript("npc_spiritcaller_dohgar") { }
bool OnGossipHello(Player* pPlayer, Creature* pCreature)
{
    if (pCreature->isQuestGiver())
        pPlayer->PrepareQuestMenu(pCreature->GetGUID());

    if (pPlayer->hasQuest(QUEST_SPIRIT_CALLING))
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_HELLO_TOTEM, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
...
bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    switch (uiAction)
    {
        case GOSSIP_ACTION_INFO_DEF+1:
            ...
            uint8 msg = pPlayer->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, ITEM_SPIRIT_CALLING_TOTEM, 1, false);
            if (msg == EQUIP_ERR_OK)
            {
                pPlayer->StoreNewItem(dest, ITEM_SPIRIT_CALLING_TOTEM, 1, true);
                pPlayer->PlayerTalkClass->ClearMenus();
            }
            break;
    }
...
void AddSC_blades_edge_mountains()
{
    ...
    new npc_ogre_brute();
new mob_lesser_nether_drake();
new npc_spiritcaller_dohgar();
}
