...
public:
    ...
    struct instance_stratholme_InstanceMapScript : public InstanceScript
    {
        ...
        bool StartSlaugtherSquare()
        {
            ...
            sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "TSCR: Instance Stratholme: Cannot open slaugther square yet.");
            return false;
        }
        ...
        void SetData(uint32 type, uint32 data)
        {
            ...
            case TYPE_BARON_RUN:
                ...
                case IN_PROGRESS:
                    ...
                    sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "TSCR: Instance Stratholme: Baron run in progress.");
                    break;
                case FAIL:
            ...
            case TYPE_RAMSTEIN:
                if (data == IN_PROGRESS)
                {
                    ...
                    if (!count)
                    {
                        ...
                        sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "TSCR: Instance Stratholme: Ramstein spawned.");
                    }
                    else
                        sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "TSCR: Instance Stratholme: %u Abomnation left to kill.",count);
                }
                ...
                if (data == DONE)
                {
                    ...
                    sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "TSCR: Instance Stratholme: Slaugther event will continue in 5 minutes.");
                }
                Encounter[4] = data;
                break;
            case TYPE_BARON:
        ...
        void Update(uint32 diff)
        {
            if (BaronRun_Timer)
            {
                if (BaronRun_Timer <= diff)
                {
                    ...
                    BaronRun_Timer = 0;
                    sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "TSCR: Instance Stratholme: Baron run event reached end. Event has state %u.",GetData(TYPE_BARON_RUN));
            ...
            if (SlaugtherSquare_Timer)
            {
                if (SlaugtherSquare_Timer <= diff)
                {
                    if (Creature* pBaron = instance->GetCreature(baronGUID))
                    {
                        ...
                        HandleGameObject(ziggurat4GUID, true);
                        HandleGameObject(ziggurat5GUID, true);
                        sLog->outDebug(LOG_FILTER_MAPSCRIPTS, "TSCR: Instance Stratholme: Black guard sentries spawned. Opening gates to baron.");
                    }
                    SlaugtherSquare_Timer = 0;
