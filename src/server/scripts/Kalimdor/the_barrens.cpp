...
#define GOSSIP_KROM_ZAR "Ou est le maitre de guerre Krom'zar?"

enum CounterAttack
{
    QUEST_COUNTER_ATTACK            = 4021,
...
const float spawnPositionHorde[5][3]=
{
    // West
    {-288.64, -1882.96, 92.61},
    {-280.87, -1879.82, 92.73},
    {-277.87, -1874.82, 92.73},
    //East
    {-246.87, -1929.82, 92.50},
    {-244.87, -1923.82, 92.50},
};

const float spawnPositionKolkar[12][3]=
{
    // West
    {-290.64, -1880.96, 92.61},
    {-285.87, -1874.82, 92.73},
    {-282.87, -1872.82, 92.73},
    {-291.64, -1878.96, 92.61},
    {-283.87, -1875.82, 92.73},
    {-281.87, -1875.82, 92.73},
    // East
    {-248.87, -1925.82, 92.50},
    {-245.87, -1926.82, 92.50},
    {-244.87, -1929.82, 92.50},
    {-247.87, -1928.82, 92.50},
    {-245.87, -1927.82, 92.50},
    {-244.87, -1928.82, 92.50},
};

const float spawnPositionKromZar[3]= {-291.70, -1857.96, 92.53};

class npc_lanti_gah : public CreatureScript
{
public:
    npc_lanti_gah() : CreatureScript("npc_lanti_gah") { }

    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_lanti_gahAI (pCreature);
    }
    struct npc_lanti_gahAI : public ScriptedAI
    {
        npc_lanti_gahAI(Creature* c) : ScriptedAI(c)
        {
            eventStarted = false ;
        }
        ...
        void JustRespawned()
        {
            m_uiResetTimer = 600000 ;
            m_uiCheckTimer = 3000 ;
            eventStarted = false ;
        }

        std::list<Creature*> kolkarList ;
        ...
        void DoAction(const int32 actionId)
        {
            if (!eventStarted)
            {
                for(uint8 i = 0 ; i<6 ; i++)
                {
                    me->SummonCreature(RAND(NPC_HORDE_DEFENDER, NPC_HORDE_AXE_THROWER), spawnPositionHorde[i][0], spawnPositionHorde[i][1], spawnPositionHorde[i][2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000);
                }

                for(uint8 i = 0 ; i<10 ; i++)
                {
                    if (Creature* kolkar = me->SummonCreature(RAND(NPC_KOLKAR_INVADER, NPC_KOLKAR_STORMSEER), spawnPositionKolkar[i][0], spawnPositionKolkar[i][1], spawnPositionKolkar[i][2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 30000))
                        kolkarList.push_back(kolkar);
                }
                m_uiResetTimer = 600000 ;
                eventStarted = true ;
            }
        ...
        void UpdateAI(const uint32 diff)
        {
            if (eventStarted)
            {
                if (m_uiCheckTimer <= diff)
                {
                    ...
                    if(!kolkarList.empty())
                    {
                        for(std::list<Creature*>::iterator itr = kolkarList.begin(); itr != kolkarList.end(); ++itr)
                        {
                            if(*itr)
                            {
                                if((*itr)->isDead())
                    ...
                    if (deadKolkar >= 7)
                    {
                        me->SummonCreature(NPC_KROM_ZAR, spawnPositionKromZar[0], spawnPositionKromZar[1], spawnPositionKromZar[2], 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000);
                        me->DisappearAndDie();
                        eventStarted = false ;
                    }
                    m_uiCheckTimer = 3000 ;
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        if (pPlayer->GetQuestStatus(QUEST_COUNTER_ATTACK) == QUEST_STATUS_INCOMPLETE)
            pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, GOSSIP_KROM_ZAR, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
