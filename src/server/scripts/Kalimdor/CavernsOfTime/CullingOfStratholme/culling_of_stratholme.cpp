...
/*
SQL:
UPDATE creature_template SET ScriptName="npc_plagued_grain" WHERE entry=30996;
...
enum Spells
{
    // Quête 13149: Dissiper les Illusions
    SPELL_ARCANE_DISRUPTION                    = 49590,
    SPELL_PLAGUED_GRAIN_CRATE_VISUAL           = 49702,
                ...
                switch(uiStep)
                {
                    ...
                    case 90:
                        if (pInstance)
                        {
                            ...
                            if (GameObject* chest = me->SummonGameObject(DUNGEON_MODE(190663, 193597), 2288.35, 1498.73, 128.414, -0.994837,  0, 0, 0, 0, 99999999))
                            {
                                chest->SetUInt32Value(GAMEOBJECT_FACTION, 35);
                                chest->RemoveFlag(GAMEOBJECT_FLAGS,GO_FLAG_INTERACT_COND);
