...
** Magrami_spectre
***/
#define SPELL_CURSE_OF_THE_FALLEN_MAGRAM    18159

class npc_magrami_spectre : public CreatureScript
{
public:
    npc_magrami_spectre() : CreatureScript("npc_magrami_spectre") { }

    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_magrami_spectreAI(pCreature);
    }
   
struct npc_magrami_spectreAI : public ScriptedAI
{
    npc_magrami_spectreAI(Creature* c) : ScriptedAI(c)
    {
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (curseTimer <= diff)
        {
            DoCast(me->getVictim(), SPELL_CURSE_OF_THE_FALLEN_MAGRAM) ;
            curseTimer = urand(15000, 18000) ;
...
enum GetMeOutOfHere
{
    ...
    NPC_HORNIZZ_BRIMBUZZLE        = 6019,
    NPC_MARAUDINE_BONEPAW        = 6070,
    NPC_MARAUDINE_WRANGLER        = 4655,
    NPC_MARAUDINE_SCOUT            = 4654,
    MELIZZA_SAY_1                = -1006000,
    MELIZZA_SAY_2                = -1006001,
    MELIZZA_SAY_3                = -1006002
};
    ...
    void WaypointReached(uint32 uiWP)
    {
        switch (uiWP)
        {
            case 0:
                ...
                break;
            case 3:
                me->SummonCreature(NPC_MARAUDINE_SCOUT, -1310, 2674, 111.6, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                me->SummonCreature(NPC_MARAUDINE_SCOUT, -1310, 2674, 111.6, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                me->SummonCreature(NPC_MARAUDINE_SCOUT, -1292, 2643, 111.6, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                me->SummonCreature(NPC_MARAUDINE_SCOUT, -1292, 2643, 111.6, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                break;
            case 5:
                SetRun(false) ;
            ...
            case 9:
                ...
                break;
            case 11:
                me->SummonCreature(NPC_MARAUDINE_BONEPAW, -1404, 2445, 88.6, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                me->SummonCreature(NPC_MARAUDINE_WRANGLER, -1403, 2445, 88.6, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                me->SummonCreature(NPC_MARAUDINE_BONEPAW, -1372, 2445, 89.82, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                me->SummonCreature(NPC_MARAUDINE_WRANGLER, -1371, 2445, 89.82, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                me->SummonCreature(NPC_MARAUDINE_BONEPAW, -1391, 2439, 88.56, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                me->SummonCreature(NPC_MARAUDINE_WRANGLER, -1390, 2439, 88.56, 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 10000) ;
                break;
            case 14:
                SetRun(false) ;
                break;
            case 18:
                ...
                break;
            case 26:
                if (Creature* hornizz = me->FindNearestCreature(NPC_HORNIZZ_BRIMBUZZLE, 5.0f))
                {
                    me->SetOrientation(2.06) ;
                    hornizzGUID = hornizz->GetGUID() ;
                    hornizz->SetOrientation(5.38) ;
                    bool isEvent = true ;
                    eventStep = 1 ;
                }
                else
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (isEvent)
        {
            if (eventTimer <= uiDiff)
            {
                ...
                {
                case 1:
                    me->Say(MELIZZA_SAY_1, LANG_UNIVERSAL, hornizzGUID) ;
                    eventTimer = 8000 ;
                    eventStep++ ;
                case 2:
                    me->Say(MELIZZA_SAY_2, LANG_UNIVERSAL, hornizzGUID) ;
                    eventTimer = 10000 ;
                    eventStep ++ ;
                case 3:
                    if (Creature* hornizz = Unit::GetCreature(*me, hornizzGUID))
                        hornizz->Say(MELIZZA_SAY_3, LANG_UNIVERSAL, hornizzGUID) ;
                    isEvent = false ;
                    if (Player* pPlayer = GetPlayerForEscort())
                        pPlayer->GroupEventHappens(QUEST_GET_ME_OUT_OF_HERE, me);
                }
