...
#define TRAIT_DE_FEU            2138
#define ECLAIR_GIVRE            837
...
public:
    ...
    struct npc_mirror_imageAI : CasterAI /*PetAI*/
    {
        npc_mirror_imageAI(Creature *c) : CasterAI(c) /*PetAI(c)*/ {}
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (me->getVictim())
            {
                if (me->ToPet() && me->_CheckTargetCC(me->getVictim()))
                {
                    me->AttackStop();
                    return;
                }
            }
            else if (me->GetOwner() && me->GetCharmInfo())
            {
    ...
    if (pPlayer->GetAchievementMgr().HasAchieved(achiPonyUp))
        if (!pCreature->HasAura(SPELL_SQUIRE_TIRED))
        {
            ...
            if (uiBuff & STATE_MAIL)
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Visit a mailbox. (Non Implementato)", GOSSIP_SENDER_MAIN, 3);
        }
    ...
    switch (uiAction)
    {
        ...
        case GOSSIP_ACTION_TRADE:
            ...
            if (!pPlayer->HasAura(SPELL_CHECK_TIRED))
                pPlayer->AddAura(SPELL_CHECK_TIRED, pPlayer);
            break;
        case 3: pCreature->MonsterSay("Not implemented.", LANG_UNIVERSAL, pPlayer->GetGUID());
            /*if (!pCreature->HasAura(SPELL_SQUIRE_POSTMAN))
...
{
    npc_argent_squireAI(Creature *c) : ScriptedAI(c)
    {
        m_current_pennant = 0;
        check_timer = 1000;
    }
    ...
    void SetData(uint32 id, uint32 spell)
    {
        if (id)
        {
            me->AddAura(spell, me);
...
{
    return new npc_argent_squireAI(pCreature);
}
};
