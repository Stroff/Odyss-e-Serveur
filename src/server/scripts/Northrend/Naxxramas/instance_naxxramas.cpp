...
class npc_tele_naxx : public CreatureScript
{
public:
    npc_tele_naxx() : CreatureScript("npc_tele_naxx") { }
CreatureAI* GetAI(Creature* pCreature)
{
    return new npc_tele_naxxAI (pCreature);
}
struct npc_tele_naxxAI : public ScriptedAI
{
    npc_tele_naxxAI(Creature *c) : ScriptedAI(c)
    {
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {    
            ...
            {
                DoCast(SPELL_VI);
                me->SummonGameObject(RAND(181576,181577,181578), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0,  0, 0, 0, 0, 99999999);
                visuel = false;
            }
            ...
            Map::PlayerList const &PlayerList = pMap->GetPlayers();
                for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                {
                    ...
                    {
                        if(me->IsWithinDist(i->getSource(), 2.0f))
                        {
                            i->getSource()->CastSpell(i->getSource(), SPELL_TE, true);
                        }
                    }
                }
        CheckTimer = 500;
