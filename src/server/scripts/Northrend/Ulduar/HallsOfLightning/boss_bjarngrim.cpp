    ...
    void Reset()
    {
            ...
            for (uint8 i = 0; i < 2; ++i)
        {
                if (Creature* pStormforgedLieutenant = (Unit::GetCreature((*me), m_auiStormforgedLieutenantGUID[i])))
            {
                ...
                pStormforgedLieutenant->GetMotionMaster()->MoveChase(me);
            }
            else if (Creature* pStormforgedLieutenant = me->SummonCreature(NPC_STORMFORGED_LIEUTENANT, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000))
            {
                m_auiStormforgedLieutenantGUID[i] = pStormforgedLieutenant->GetGUID();
                pStormforgedLieutenant->GetMotionMaster()->MoveChase(me);
            }
        }
    ...
    void EnterCombat(Unit* pWho)
    {
        ...
        for(uint8 i = 0; i<2; i++)
            if (Creature* pStormforgedLieutenant = ((Creature*)Unit::GetUnit((*me), m_auiStormforgedLieutenantGUID[i])))
                pStormforgedLieutenant->AI()->AttackStart(pWho);
