...
enum Spells
{
    ...
    SPELL_BLOCK_OF_ICE                          = 45776, // 61969,
	SPELL_BLOCK_OF_ICE_AURA                     = 63985, // 28522,
...
#define SPELL_STORM_CLOUD_AURA RAID_MODE(63711, 65134)
...
#define ACHIEVEMENT_PRE_POUR_LHIVER             RAID_MODE(2969, 2970)

enum eActions
{
	ACTION_FAILED_COOLEST_FRIENDS      = 1 // achievement
};

enum NPCs
{
    NPC_FLASH_FREEZE_PRE                        = 32926,
    NPC_FLASH_FREEZE                            = 32926, // 32938
...
#define EMOTE_FREEZE        "Hodir commence a incanter Gel instantane !"
#define EMOTE_BLOWS         "Hodir gagne Coups geles !"

enum GameObjects
{
    CACHE_OF_WINTER_10                          = 194307,
    CACHE_OF_WINTER_25                          = 194308,
	Hodir_Rare_CHEST_10                         = 194200,
	Hodir_Rare_CHEST_25                         = 194201,
	GO_HODIR_DOOR_0                             = 194442, // Before Hodir
	GO_HODIR_DOOR_1                             = 194441, // After
	GO_HODIR_DOOR_2                             = 194634, // After
};
...
public:
    ...
    {
        npc_flash_freeze_preAI(Creature *c) : ScriptedAI(c)
        {
            FrostTombGUID = 0;
        }
        ...
        void JustDied(Unit *killer)
        {
            ...
            if (FrostTombGUID)
            {
                ...
                if (FrostTomb)
				{
                    FrostTomb->RemoveAurasDueToSpell(SPELL_BLOCK_OF_ICE);
					FrostTomb->RemoveAurasDueToSpell(SPELL_BLOCK_OF_ICE_AURA);
				}
            }
        }
  };
...
struct boss_hodir_AI : public BossAI
{
    boss_hodir_AI(Creature *pCreature) : BossAI(pCreature, TYPE_HODIR)
    {
		...
		me->ApplySpellImmune(0, IMMUNITY_ID, SPELL_BLOCK_OF_ICE, true);
    }
    ...
    void Reset()
    {
        _Reset();
		 ...
		 if (GameObject* Door0 = me->FindNearestGameObject(GO_HODIR_DOOR_0, 500.0f))
			 if (Door0->GetGoState() != GO_STATE_ACTIVE)
				pInstance->DoUseDoorOrButton(Door0->GetGUID());
    }
    ...
    void EnterCombat(Unit* who)
    {
		...
		if (GameObject* Door0 = me->FindNearestGameObject(GO_HODIR_DOOR_0, 100.0f))
			pInstance->DoUseDoorOrButton(Door0->GetGUID());

        for (int i = 0; i < RAID_MODE(NORMAL_COUNT, RAID_COUNT); i++)
        {
			...
			{
				if (Creature *pChains = me->SummonCreature(NPC_FLASH_FREEZE, pHelper->GetPositionX(), pHelper->GetPositionY(), pHelper->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 200000))
				{
					CAST_AI(npc_flash_freeze_pre::npc_flash_freeze_preAI, pChains->AI())->SetPrisoner(pHelper);
					pChains->CastSpell(pHelper, SPELL_BLOCK_OF_ICE, true);
					pChains->AddAura(SPELL_BLOCK_OF_ICE_AURA, pHelper);
				}
    ...
    void JustDied(Unit *victim)
    {
		...
		me->setFaction(35);
		me->SetVisible(false);
		if (Creature* HodirImage = me->SummonCreature(NPC_HODIR_IMAGE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN))
		{
			HodirImage->SetVisible(true);
			HodirImage->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
			HodirImage->ForcedDespawn(15000);
			HodirImage->Say(SAY_DEATH, 0, HodirImage->GetGUID());
		}
		...
		for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
		{
			...
			if (pPlayer->HasAura(SPELL_BLOCK_OF_ICE_AURA))
			{
				pPlayer->RemoveAurasDueToSpell(SPELL_BLOCK_OF_ICE);
				pPlayer->RemoveAurasDueToSpell(SPELL_BLOCK_OF_ICE_AURA);
			}
		}
        ...
        if (pInstance)
        {
			if (GameObject* Door0 = me->FindNearestGameObject(GO_HODIR_DOOR_0, 500.0f))
				pInstance->DoUseDoorOrButton(Door0->GetGUID());
			if (GameObject* Door1 = me->FindNearestGameObject(GO_HODIR_DOOR_1, 500.0f))
				pInstance->DoUseDoorOrButton(Door1->GetGUID());
			if (GameObject* Door2 = me->FindNearestGameObject(GO_HODIR_DOOR_2, 500.0f))
				pInstance->DoUseDoorOrButton(Door2->GetGUID());
            ...
            if (RareCache)
            {
				...
				me->SummonGameObject(RAID_MODE(Hodir_Rare_CHEST_10, Hodir_Rare_CHEST_25), 2037.08f, -200.9482f, 432.687f, 3.13054f, 0, 0, 0.7f, 0.7f, 604800);
    ...
    void UpdateAI(const uint32 diff)
    {
		...
		AchievementEntry const *achievWhatTheEck = GetAchievementStore()->LookupEntry(ACHIEVEMENT_PRE_POUR_LHIVER);
		if (achievWhatTheEck)
		{
			Map::PlayerList const &players = pInstance->instance->GetPlayers();
			for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
				if (itr->getSource()->HasAura(SPELL_STORM_CLOUD_AURA)) // Puissance de la tempete
					if (itr->getSource()->HasAura(SPELL_SINGED)) // Feu douillet
						if (itr->getSource()->HasAura(SPELL_STARLIGHT)) // Lumière stellaire
        ...
        while(uint32 eventId = events.ExecuteEvent())
        {
            switch(eventId)
            {
                ...
                case EVENT_RARE_CACHE:
					me->TextEmote(SAY_HARD_MODE_MISSED, 0, true);
                    RareCache = false;
    ...
    void FlashFreeze()
    {
        ...
        for (std::list<HostileReference*>::const_iterator itr = ThreatList.begin(); itr != ThreatList.end(); ++itr)
        {
            ...
            {
                if (pTarget->HasAura(SPELL_BLOCK_OF_ICE_AURA))
                {
                ...
                else
                {
                    if (GetClosestCreatureWithEntry(pTarget, NPC_ICICLE_TARGET, 5.0f))
                        ...
                        if (Creature *pChains = me->SummonCreature(NPC_FLASH_FREEZE, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 200000))
                        {
                            CAST_AI(npc_flash_freeze_pre::npc_flash_freeze_preAI, pChains->AI())->SetPrisoner(pTarget);
                            pChains->CastSpell(pTarget, SPELL_BLOCK_OF_ICE, true);
							pChains->AddAura(SPELL_BLOCK_OF_ICE_AURA, pTarget);
                        }
