...
#define SPELL_LIGHTNING_WHIRL_DAMAGE          RAID_MODE(61916,63482)
...
#define EMOTE_OVERLOAD              "Mande-foudre Brundir commence a incanter une surcharge !"
...
public:
    ...
    struct boss_steelbreakerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			...
			if (Creature* npcenrage = me->FindNearestCreature(93290, 50, true))
				npcenrage->DisappearAndDie();
        }
        ...
        void EnterCombat(Unit* who)
        {
         ...
         //   events.ScheduleEvent(EVENT_BERSERK, 900000);
			...
			if (Creature* npcenrage = me->SummonCreature(93290, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()))
				npcenrage->SetInCombatWithZone();
        }
        ...
        void JustDied(Unit* /*Killer*/)
        {
            ...
            if (IsEncounterComplete(pInstance, me) && pInstance)
            {
				...
				if (pInstance->GetData(EVENT_BRUNDIR_STUNNED) == DONE)
					pInstance->DoCompleteAchievement(ACHIEVEMENT_CANT_WHILE_STUNNED);

				if (Creature* npcenrage = me->FindNearestCreature(93290, 50, true))
					npcenrage->DisappearAndDie();

				AchievementEntry const *achievWhatTheEck = GetAchievementStore()->LookupEntry(ACHIEVEMENT_ON_YOUR_SIDE);
				if (achievWhatTheEck)
				{
					Map::PlayerList const &players = pInstance->instance->GetPlayers();
					for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
						if (itr->getSource()->HasAura(58501)) // spell morph botte en fer
        ...
        void DoAction(const int32 action)
        {
            switch (action)
            {
                case ACTION_MOLGEIM:
                    ...
                    // Add HardMode Loot
                    // me->AddLootMode(LOOT_MODE_HARD_MODE_1); // n'a pas d'ajout de loots en mode moyen
                    break;
				case DO_ENTER_ENRAGE:
                DoCast(me, SPELL_BERSERK, true);
...
public:
    ...
    struct boss_stormcaller_brundirAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			...
			{
                pInstance->SetBossState(TYPE_ASSEMBLY, NOT_STARTED);
				pInstance->SetData(EVENT_BRUNDIR_STUNNED, NOT_STARTED);
			}
            me->RemoveAllAuras();
            me->RemoveUnitMovementFlag(MOVEMENTFLAG_LEVITATING | MOVEMENTFLAG_WALKING);
            events.Reset();
            phase = 0;
			cantwhilestunned = true;
        ...
        {
            if (pWho && pWho->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_CHAIN_LIGHTNING)
                cantwhilestunned = false;
            if (pWho && pWho->GetTypeId() == TYPEID_PLAYER && pSpell->Id == SPELL_LIGHTNING_WHIRL_DAMAGE)
                cantwhilestunned = false;
        }
        ...
        void EnterCombat(Unit* who)
        {
			...
			if (pInstance)
				pInstance->SetData(EVENT_BRUNDIR_STUNNED, IN_PROGRESS);
        }
        ...
        void JustDied(Unit* /*Killer*/)
        {
            ...
            else
			{
				...
				if (cantwhilestunned && pInstance)
					pInstance->SetData(EVENT_BRUNDIR_STUNNED, DONE);
				else if (pInstance)
					pInstance->SetData(EVENT_BRUNDIR_STUNNED, FAIL);
			}
...
class npc_enrage : public CreatureScript
{
public:
    npc_enrage() : CreatureScript("npc_enrage") { }

    CreatureAI *GetAI(Creature* pCreature) const
    {
        return new npc_enrageAI(pCreature);
    }

    struct npc_enrageAI : public Scripted_NoMovementAI
    {
        npc_enrageAI(Creature *c) : Scripted_NoMovementAI(c)
        {
            pInstance = c->GetInstanceScript();
			me->SetReactState(REACT_PASSIVE);
        }
		...
		void UpdateAI(const uint32 diff)
        {
            ...
            {
			if (Creature* Steelbreaker = me->GetCreature(*me, pInstance->GetData64(DATA_STEELBREAKER)))
				Steelbreaker->AI()->DoAction(DO_ENTER_ENRAGE);
            if (Creature* Brundir = me->GetCreature(*me, pInstance->GetData64(DATA_BRUNDIR)))
				Brundir->AI()->DoAction(DO_ENTER_ENRAGE);
            if (Creature* Molgeim = me->GetCreature(*me, pInstance->GetData64(DATA_MOLGEIM)))
				Molgeim->AI()->DoAction(DO_ENTER_ENRAGE);
            Enraged = true;
            }
