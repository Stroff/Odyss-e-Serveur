...
enum eCreatures
{
    NPC_IRON_CONSTRUCT                          = 331210, // 33121, même id que les statues, pose des problemes sur le stack de chaleur.
	NPC_STATUE_IRON                             = 33121,
...
#define SPELL_BRITTLE                   RAID_MODE(62382,67114)
...
public:
    ...
    struct boss_ignis_AI : public BossAI
    {
        ...
        bool Shattered;
		bool Stokin_the_Furnace; // Achievement Timer
        uint64 SlagPotGUID;
        ...
        void EnterCombat(Unit* /*who*/)
        {
			...
			events.ScheduleEvent(EVENT_FURNACE, 240000);
            events.ScheduleEvent(EVENT_BERSERK, 480000);
            SlagPotGUID = 0;
            ConstructTimer = 0;
            Shattered = false;
			Stokin_the_Furnace = true;
        }
            ...
            while(uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_CONSTRUCT:
                        ...
                        break;
					case EVENT_FURNACE:
						Stokin_the_Furnace = false;
						events.CancelEvent(EVENT_FURNACE);
						break;
                    case EVENT_BERSERK:
...
public:
    ...
    struct npc_iron_constructAI : public ScriptedAI
    {
        npc_iron_constructAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
			...
			if (me->GetEntry() == NPC_STATUE_IRON)
			{
				me->setFaction(35);
...
public:
    ...
    struct npc_scorch_groundAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
            if (!Heat)
            {
                if (pWho->GetEntry() == NPC_IRON_CONSTRUCT && me->IsWithinDistInMap(pWho, 10.0f))
                {
