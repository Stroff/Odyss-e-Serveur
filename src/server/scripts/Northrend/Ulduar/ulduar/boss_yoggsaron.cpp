...
/* ScriptData
SDName: Yogg-Saron
SDAuthor: Darkman, edited by Eracus.
EndScriptData */

/* Todo:
- malady of mind fear need check
- check SelectTarget for spells...
- check visibility for keepers
*/
...
#define GOSSIP_KEEPER_HELP           "Nous avons besoin de votre aide."
...
enum Achievments
{
	...
	ACHIEVMENT_COMING_OUT_OF_THE_WALLS_10       = 3014, // just need to apply SQL
	ACHIEVMENT_COMING_OUT_OF_THE_WALLS_25       = 3017, // just need to apply SQL
...
#define ACHIEVMENT_KISS_AND_MAKE_UP   RAID_MODE(ACHIEVMENT_KISS_AND_MAKE_UP_10,ACHIEVMENT_KISS_AND_MAKE_UP_25)
...
enum Entrys
{
    ...
    ENTRY_IMAGE_OF_FREYA                        = 33241,
    ENTRY_IMAGE_OF_THORIM                       = 33242,
    ENTRY_IMAGE_OF_MIMIRON                      = 33244,
    ENTRY_IMAGE_OF_HODIR                        = 33213,
...
enum TentacleType // support for switch id
{
    CRUSHER_TENTACLE = 0,
...
enum Spells
{
    SPELL_IN_THE_MAWS_OF_THE_OLD_GOD            = 64184, // Val'anyr
    //All Phases
    // Keeper Freya
	...
	SPELL_PSYCHOSIS_2                           = 63795, // don't use ? more damage but less reduces sanity
	...
	SPELL_MALADY_OF_MIND_DUMP                   = 63881,
...
public:
    ...
    struct boss_saraAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            uiRandomYell_Timer = urand(300000,600000); // 5-10 mns
				...
				if (!players.isEmpty())
					for(Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
					{
						if (Player* plr = itr->getSource())
							if(!plr->isAlive()) // forced remove on reset
								plr->RemoveAurasDueToSpell(SPELL_SANITY);
							else if (plr->HasAura(RAID_MODE(SPELL_SQUEEZE_10, SPELL_SQUEEZE_25)))
								plr->RemoveAurasDueToSpell(RAID_MODE(SPELL_SQUEEZE_10, SPELL_SQUEEZE_25));
					}
			}
        }
		...
		void SetYoggLootMode()
		{
			if (Creature* yogg = Creature::GetCreature((*me), m_pInstance->GetData64(TYPE_YOGGSARON)))
			{
				if (KeeperCount == 4 || KeeperCount == 3 || KeeperCount == 2)
					yogg->AddLootMode(1);
				if (KeeperCount == 1)
					yogg->AddLootMode(2);
				if (KeeperCount == 0)
        ...
        void DoSpawnKeeperForSupport(bool despawn) // Despawn on Sara Reset
        {
            if(m_pInstance)
            {
				if (!despawn)
				{
					if (Creature* Mimiron = me->FindNearestCreature(ENTRY_IMAGE_OF_MIMIRON, 1000, true))
						if (Mimiron->HasAura(SPELL_KEEPER_ACTIVE))
						{
							KeeperCount++;
							DoSummon(ENTRY_KEEPER_MIMIRON,KeeperSpawnLocation[0],0,TEMPSUMMON_MANUAL_DESPAWN);
						}
					if (Creature* Freya = me->FindNearestCreature(ENTRY_IMAGE_OF_FREYA, 1000, true))
						if (Freya->HasAura(SPELL_KEEPER_ACTIVE))
						{
							KeeperCount++;
							DoSummon(ENTRY_KEEPER_FREYA,KeeperSpawnLocation[1],0,TEMPSUMMON_MANUAL_DESPAWN);
						}
					if (Creature* Thorim = me->FindNearestCreature(ENTRY_IMAGE_OF_THORIM, 1000, true))
						if (Thorim->HasAura(SPELL_KEEPER_ACTIVE))
						{
							KeeperCount++;
							DoSummon(ENTRY_KEEPER_THORIM,KeeperSpawnLocation[2],0,TEMPSUMMON_MANUAL_DESPAWN);
						}
					if (Creature* Hodir = me->FindNearestCreature(ENTRY_IMAGE_OF_HODIR, 1000, true))
				...
				{
					if (Creature* Mimiron = me->FindNearestCreature(ENTRY_KEEPER_MIMIRON, 100, true))
						Mimiron->ForcedDespawn();
					if (Creature* Freya = me->FindNearestCreature(ENTRY_KEEPER_FREYA, 100, true))
						Freya->ForcedDespawn();
					if (Creature* Thorim = me->FindNearestCreature(ENTRY_KEEPER_THORIM, 100, true))
						Thorim->ForcedDespawn();
					if (Creature* Hodir = me->FindNearestCreature(ENTRY_KEEPER_HODIR, 100, true))
						Hodir->ForcedDespawn();
				}
            }
        }
		...
		uint32 CountKeepersActive()
        {
            ...
            if(m_pInstance)
            {
				if (Creature* Mimiron = me->FindNearestCreature(ENTRY_IMAGE_OF_MIMIRON, 100, true))
					if (Mimiron->HasAura(SPELL_KEEPER_ACTIVE))
						count++;
				if (Creature* Freya = me->FindNearestCreature(ENTRY_IMAGE_OF_FREYA, 100, true))
					if (Freya->HasAura(SPELL_KEEPER_ACTIVE))
						count++;
				if (Creature* Thorim = me->FindNearestCreature(ENTRY_IMAGE_OF_THORIM, 100, true))
					if (Thorim->HasAura(SPELL_KEEPER_ACTIVE))
						count++;
				if (Creature* Hodir = me->FindNearestCreature(ENTRY_IMAGE_OF_HODIR, 100, true))
					if (Hodir->HasAura(SPELL_KEEPER_ACTIVE))
						count++;
            }
            return count;
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            uiPsychosis_Timer = urand(5000,6000);
			uiMindSpell_Timer = urand (23000, 27000);
        ...
        void ReceiveEmote(Player* pPlayer, uint32 emote)
        {
            if(m_Phase >= PHASE_BRAIN)
            {
                ...
                {
					const AchievementEntry * Kiss= GetAchievementStore()->LookupEntry(ACHIEVMENT_KISS_AND_MAKE_UP);
                    if(pPlayer->GetAchievementMgr().HasAchieved(Kiss))
                        return;
        ...
        void DoAction(const int32 action)
        {
            ...
            case ACTION_YOGGSARON_KILLED:
                ...
                if(uiEnrage_Timer >= 480000) // fast kill
        ...
        void MoveInLineOfSight(Unit* target)
        {
            ...
            {
                if(target && me->GetDistance2d(target) <= 30.0f && target->ToPlayer() && !target->ToPlayer()->isGameMaster() && me->IsWithinLOSInMap(target) && !target->HasAura(SPELL_FLASH_FREEZE))
                {
        ...
        void DoMadness()
        {
            if(m_pInstance)
            {
                m_pInstance->HandleGameObject(m_pInstance->GetData64(DATA_BRAIN_DOOR_1),false);
                m_pInstance->HandleGameObject(m_pInstance->GetData64(DATA_BRAIN_DOOR_2),false);
                m_pInstance->HandleGameObject(m_pInstance->GetData64(DATA_BRAIN_DOOR_3),false);
            }
                        ...
                        if(uiSpeaking_Timer <= diff)
                        {
                            ...
                            case 4:
                                ...
                                if(Creature* yogg = me->GetCreature(*me,guidYogg))
                                {
									...
									yogg->CastSpell(yogg, SPELL_ERUPT, true); // just for dump players
                        ...
                        if(!me->HasAura(SPELL_SHATTERED_ILLUSIONS))
                        {
                            if(currentBrainEventPhase != PORTAL_PHASE_BRAIN_NONE)
                            {
                                if(AllSpawnsDeadOrDespawned(guidEventTentacles))
                                {
                                    switch(currentBrainEventPhase)
                                    {
                                    case PORTAL_PHASE_DRAGON_SOUL: if(m_pInstance) m_pInstance->HandleGameObject(m_pInstance->GetData64(DATA_BRAIN_DOOR_1),true); break;
                                    case PORTAL_PHASE_LICH_KING: if(m_pInstance) m_pInstance->HandleGameObject(m_pInstance->GetData64(DATA_BRAIN_DOOR_2),true); break;
                                    case PORTAL_PHASE_KING_LLIANE: if(m_pInstance) m_pInstance->HandleGameObject(m_pInstance->GetData64(DATA_BRAIN_DOOR_3),true); break;
                                    }
                                    currentBrainEventPhase = PORTAL_PHASE_BRAIN_NONE;
                                    IsEventSpeaking = false;
                                    if(Creature* yogg = me->GetCreature(*me,guidYogg))
                            ...
                            if(yogg->IsNonMeleeSpellCasted(false))
                            {
                                if(Creature* guard = GetRandomGuardianTarget())
                                {
                                    if(guard->HasAura(SPELL_WEAKENED))
                                            yogg->AddAura(SPELL_SHADOW_BEACON, guard);
                                }
		...
		void spawnTentacles()
        {
            ...
            {
                case 0:
                    me->SummonCreature(ENTRY_CRUSHER_TENTACLE, TentaclesPos[rand()%22]);
                case 1:
                case 2:
                    me->SummonCreature(ENTRY_CORRUPTOR_TENTACLE, TentaclesPos[rand()%22]);
                    if (Player* pTarget = SelectPlayerTargetInRange(500.0f))
						if(!IsPlayerInBrainRoom(pTarget) && !pTarget->HasAura(SPELL_INSANE))
						{
							Position pos;
							pTarget->GetPosition(&pos);
							if (Creature* Constrict = me->SummonCreature(ENTRY_CONSTRICTOR_TENTACLE, pos))
								pTarget->_EnterVehicle(Constrict->GetVehicleKit(), 0);
                        }
                    break;
            }
...
public:
    ...
    struct npc_ominous_cloudAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* target)
        {
			if(pInstance && pInstance->GetBossState(TYPE_YOGGSARON) == IN_PROGRESS)
...
public:
    ...
    struct npc_guardian_of_yogg_saronAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            if(m_pInstance)
            {
                if(Creature* cSara = me->GetCreature(*me,m_pInstance->GetData64(DATA_SARA)))
				{
...
public:
    ...
    struct npc_yogg_saron_tentacleAI : public Scripted_NoMovementAI
    {
        npc_yogg_saron_tentacleAI(Creature *c) : Scripted_NoMovementAI(c)
        {
            ...
            me->SetFlying(true);
            if(m_pInstance)
                if(Creature* sara = Creature::GetCreature(*me,m_pInstance->GetData64(DATA_SARA)))
        ...
        void SetTentacleType(uint32 entry)
        {
            switch(entry)
            {
				case ENTRY_CRUSHER_TENTACLE : t_Type = CRUSHER_TENTACLE;
					break;
				case ENTRY_CORRUPTOR_TENTACLE : t_Type = CORRUPTOR_TENTACLE;
					break;
				default: t_Type = CORRUPTOR_TENTACLE;
					break;
            }
        }
...
public:
    ...
    struct npc_constrictor_tentacle_AI : public Scripted_NoMovementAI
    {
		...
		void JustDied(Unit* /*killer*/)
        {
			...
			{
				target->RemoveAurasDueToSpell(RAID_MODE(SPELL_SQUEEZE_10, SPELL_SQUEEZE_25));
				target->ExitVehicle();
			}
            me->RemoveAurasDueToSpell(SPELL_TENTACLE_VOID_ZONE);
            me->DespawnOrUnsummon(5000);
		...
		void UpdateAI(const uint32 diff)
        {
			...
			if (!Squeezed)
			{
				if (Unit* pTarget = me->GetVehicleKit()->GetPassenger(0))
					me->AddAura(RAID_MODE(SPELL_SQUEEZE_10, SPELL_SQUEEZE_25), pTarget);
				Squeezed = true;
			}
        }
    };
                            ...
                            if (Player* plr = itr->getSource())
                                if(plr->isAlive() && !plr->isGameMaster())
								{
                                    if(!plr->HasAura(SPELL_SANITY) && !plr->HasAura(SPELL_INSANE))
                                    {
										...
										// for achievement drive_me_crazy
									...
									if (!plr->HasAura(SPELL_SANITY))
										PlayerSanityCount++;
								}
						}
						if (PlayerSanityCount == PlayerList.size()) // don't continue if all player are crazy
						{
							for(Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
								if (Player* plr = itr->getSource())
									if(plr->isAlive() && !plr->isGameMaster())
										me->Kill(plr, true);
							// reset sara and yogg-saron
							if(Creature* cSara = me->GetCreature(*me,m_pInstance->GetData64(DATA_SARA)))
								cSara->AI()->EnterEvadeMode();
							EnterEvadeMode();
						}
                }
				PlayerList.size() == NULL;
				PlayerSanityCount = 0;
                uiSanityCheck_Timer = 1000;
                ...
                case ENTRY_KEEPER_MIMIRON:
                    {
						...
						{
						case 0:
							if(Unit* tentacules = GetRandomEntryTarget(ENTRY_CRUSHER_TENTACLE,500.0f))
								if (!tentacules->HasAura(SPELL_DESTABILIZATION_MATRIX))
									me->CastSpell(tentacules, SPELL_DESTABILIZATION_MATRIX, true);
							break;
						case 1:
							if(Unit* tentacules = GetRandomEntryTarget(ENTRY_CORRUPTOR_TENTACLE,500.0f))
								if (!tentacules->HasAura(SPELL_DESTABILIZATION_MATRIX))
									me->CastSpell(tentacules, SPELL_DESTABILIZATION_MATRIX, true);
							break;
						case 2:
							if(Unit* tentacules = GetRandomEntryTarget(ENTRY_CONSTRICTOR_TENTACLE,500.0f))
								if (!tentacules->HasAura(SPELL_DESTABILIZATION_MATRIX))
									me->CastSpell(tentacules, SPELL_DESTABILIZATION_MATRIX, true);
							break;
						}
                        ...
                        if (Player* player = itr->getSource())
							if(player->HasAura(SPELL_BRAIN_LINK) && player->GetGUID() != trigger->GetGUID())
								if (player->IsWithinDist(trigger, 20))
									trigger->CastSpell(player, SPELL_BRAIN_LINK_DUMMY, true, 0, 0);
								else
									trigger->CastSpell(player, SPELL_BRAIN_LINK_DAMAGE, true, 0, 0);
                }
            }
        }
...
public:
    ...
    class spell_malady_of_the_mindScript : public AuraScript
    {
        PrepareAuraScript(spell_malady_of_the_mindScript);
        ...
        void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* trigger = GetTarget())
            {
                if (trigger->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    trigger->GetRaidMember(unitList, 10);
                    for (std::list<Unit*>::iterator itr = unitList.begin(); itr != unitList.end(); ++itr)
                    {
						...
						pUnit->CastSpell(pUnit, SPELL_MALADY_OF_MIND_DUMP, true, 0, 0);
                        return;
					}
				}
			}
        ...
        void Register()
        {
            OnEffectRemove += AuraEffectRemoveFn(spell_malady_of_the_mindScript::OnRemove, EFFECT_0, SPELL_AURA_MOD_FEAR, AURA_EFFECT_HANDLE_REAL);
			// Spell 63830 Effect `Index: EFFECT_0 AuraName: 7` of script did not match dbc effect data - bound handler won't be executed
        }
    ...
    AuraScript* GetAuraScript() const
    {
        return new spell_malady_of_the_mindScript();
    }
};
