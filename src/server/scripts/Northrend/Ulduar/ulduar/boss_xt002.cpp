...
#define EMOTE_TYMPANIC    "Le Deconstructeur XT-002 commence a faire trembler la terre.."
#define EMOTE_HEART       "Le Coeur du Deconstructeur XT-002 est expose et libere de l'energie."
#define EMOTE_REPAIR      "Le Deconstructeur XT-002 consume un Robot-ferrailleur pour se regenerer."
    ...
    struct boss_xt002_AI : public BossAI
    {
        ...
        void Reset()
        {
            ...
            if (instance)
			{
				...
				if (GameObject* XT002Door = me->FindNearestGameObject(GO_XT_002_DOOR, 500.0f))
					if (XT002Door->GetGoState() != GO_STATE_ACTIVE)
						instance->DoUseDoorOrButton(XT002Door->GetGUID());
			}
        }
        ...
        void exposeHeart()
        {
            ...
            if (Creature* Heart = me->SummonCreature(NPC_XT002_HEART, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()+2, 0, TEMPSUMMON_TIMED_DESPAWN, TIMER_HEART_PHASE))
                Heart->_EnterVehicle(me->GetVehicleKit(), 1);
...
public:
    ...
    struct mob_life_sparkAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (m_pInstance && m_pInstance->GetBossState(TYPE_XT002) != IN_PROGRESS)
                me->ForcedDespawn();
