...
#define EMOTE_LEVIATHAN      "Leviathan MK II commence a lancer Explosion de plasma !"
...
enum eSpells
{
    ...
    // VX-001
   // SPELL_RAPID_BURST                           = 63382, // ne fonctionne pas
    SPELL_RAPID_BURST_10                        = 63387,
	SPELL_RAPID_BURST_25                        = 64531,
	SPELL_RAPID_BURST_D_10                      = 64019,
	SPELL_RAPID_BURST_D_25                      = 64532,
	...
	SPELL_HAND_PULSE_D_10                       = 64352,
    SPELL_HAND_PULSE_D_25                       = 64537,
    ...
    SPELL_BOOM_BOT                              = 63801, // ne fonctionne pas
...
enum eEvents
{
    ...
    EVENT_LASER_BARRAGE,
	EVENT_LASER_BARRAGE_ASSEMBLED,
    EVENT_ROCKET_STRIKE,
	EVENT_ROCKET_STRIKE_2,
    EVENT_HEAT_WAVE,
    EVENT_HAND_PULSE,
	EVENT_HAND_PULSE_D,
    EVENT_FROST_BOMB,
...
enum eActions
{
	...
	STRIKE_BOMBE                                = 14 // achievement set_up_us_the_bomb
};

enum Npcs
{
	NPC_TURRET_LEVIATH                          = 34071,
	NPC_ROCKET                                  = 34050,
	NPC_MINE                                    = 34362,
...
enum MimironChests
{
    ...
    CACHE_OF_INNOVATION_HARDMODE_25             = 194790
};
...
struct boss_mimironAI : public BossAI
{
    ...
    void Reset()
    {
		...
		if (!FirstTry)
            pInstance->SetBossState(TYPE_MIMIRON, FAIL);

		if (MimironHardMode)
			if (GameObject* pButton = me->FindNearestGameObject(194739,500.0f))
		...
		DespawnCreatures(NPC_TURRET_LEVIATH, 100);
    }
    ...
    void JustDied(Unit *victim)
    {
		...
		me->setFaction(35);
		me->SetVisible(false);
		if (Creature* MimironImage = me->SummonCreature(NPC_MIMIRON_IMAGE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN))
		{
			MimironImage->SetVisible(true);
			MimironImage->SetDisplayId(me->GetDisplayId()); // else Mimiron Image is to big
			MimironImage->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
			MimironImage->ForcedDespawn(15000);
			MimironImage->Say(SAY_V07TRON_DEATH, 0, MimironImage->GetGUID());
		}
        ...
        if (pInstance)
        {
			pInstance->SetBossState(TYPE_MIMIRON, DONE);
            if (MimironHardMode)
            {
			...
			if (!StrikeBombe)
				pInstance->DoCompleteAchievement(ACHIEVEMENT_SET_UP_US_THE_BOMB);
        }
    }
                                ...
                                if (pVX_001->getStandState() == UNIT_STAND_STATE_DEAD)
                                    if (pAerialUnit->getStandState() == UNIT_STAND_STATE_DEAD)
                                    {
                                        if (Unit* pTarget = me->SelectNearestTarget())
                                            pTarget->ToPlayer()->RewardPlayerAndGroupAtKill(pLeviathan,true);
            ...
            if (uiPhase_timer <= diff)
            {
                switch (uiStep)
                {
                    ...
                    case 3:
                        if (pInstance)
                            if (Creature *pLeviathan = Creature::GetCreature((*me), pInstance->GetData64(DATA_LEVIATHAN_MK_II)))
                                me->_EnterVehicle(pLeviathan->GetVehicleKit(), 2);
                        JumpToNextStep(2000);
                        break;
                    case 4:
                        if (pInstance)
                            if (Creature *pLeviathan = Creature::GetCreature((*me), pInstance->GetData64(DATA_LEVIATHAN_MK_II)))
                                me->_EnterVehicle(pLeviathan->GetVehicleKit(), 5);
                    ...
                    case 6:
                        if (pInstance)
                            if (Creature *pLeviathan = Creature::GetCreature((*me), pInstance->GetData64(DATA_LEVIATHAN_MK_II)))
                                me->_EnterVehicle(pLeviathan->GetVehicleKit(), 6);
                        JumpToNextStep(2000);
                        break;
                    case 7:
                        if (pInstance)
                ...
                switch (uiStep)
                {
                    ...
                    case 2:
                        if (pInstance)
                            if (Creature *pLeviathan = Creature::GetCreature((*me), pInstance->GetData64(DATA_LEVIATHAN_MK_II)))
							{
                                me->_EnterVehicle(pLeviathan->GetVehicleKit(), 1);
								pLeviathan->GetMotionMaster()->MovePoint(0, 2790.11f, 2595.83f, 364.32f);
							}
                        JumpToNextStep(2000);
                        break;
                    case 3:
                    ...
                    case 4:
                        if (pInstance)
                            if (Creature *pVX_001 = Creature::GetCreature((*me), pInstance->GetData64(DATA_VX_001)))
                            {
                                ...
                                if (Creature* Rocket1 = pVX_001->SummonCreature(NPC_ROCKET, pVX_001->GetPositionX(), pVX_001->GetPositionY(), pVX_001->GetPositionZ(), 0, TEMPSUMMON_MANUAL_DESPAWN))
                                    Rocket1->_EnterVehicle(pVX_001->GetVehicleKit(), 5);
                                if (Creature* Rocket2 = pVX_001->SummonCreature(NPC_ROCKET, pVX_001->GetPositionX(), pVX_001->GetPositionY(), pVX_001->GetPositionZ(), 0, TEMPSUMMON_MANUAL_DESPAWN))
                                    Rocket2->_EnterVehicle(pVX_001->GetVehicleKit(), 6);
                            }
                        JumpToNextStep(8000);
                        break;
                    case 5:
            ...
            if (uiPhase_timer <= diff)
            {
                switch (uiStep)
                {
                    case 1:
                        if (pInstance)
                            if (Creature *pVX_001 = Creature::GetCreature((*me), pInstance->GetData64(DATA_VX_001)))
                                me->_EnterVehicle(pVX_001->GetVehicleKit(), 4);
                        me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_STAND);
                        JumpToNextStep(2500);
                        break;
                    case 2:
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
						if (Creature* pAerial = Creature::GetCreature((*me), pInstance->GetData64(DATA_AERIAL_UNIT)))
							me->_EnterVehicle(pAerial->GetVehicleKit(), 0);
						me->SetVisible(false); // wrong Seatid for Aerial unit
                        me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_TALK);
            ...
            if (uiPhase_timer <= diff)
            {
                switch (uiStep)
                {
					case 1:
                        if (pInstance)
                            if (Creature *pLeviathan = Creature::GetCreature((*me), pInstance->GetData64(DATA_LEVIATHAN_MK_II)))
								pLeviathan->GetMotionMaster()->MovePoint(0, 2755.526f, 2569.46f, 364.397f);
                        JumpToNextStep(2000);
                        break;
					case 2:
                        if (pInstance)
...
struct boss_leviathan_mkAI : public BossAI
{
    ...
    void KilledUnit(Unit *who)
    {
        if (!(rand()%5))
            if (pInstance)
                if (Creature *pMimiron = Creature::GetCreature((*me), pInstance->GetData64(TYPE_MIMIRON)))
                {
    ...
    void DamageTaken(Unit *who, uint32 &damage)
    {
        if (phase == PHASE_LEVIATHAN_SOLO)
            if (damage >= me->GetHealth())
            {
                ...
                if (Creature *turret = CAST_CRE(me->GetVehicleKit()->GetPassenger(7)))
				{
                    turret->Kill(turret, false);
					turret->SetVisible(false);
				}
                me->SetSpeed(MOVE_RUN, 1.5f, true);
				me->GetMotionMaster()->MovePoint(0, 2794.48f, 2598.31f, 364.32f);
            }
    ...
    void EnterCombat(Unit *who)
    {
		...
		if (Creature* pTurret = (me->SummonCreature(NPC_TURRET_LEVIATH, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_MANUAL_DESPAWN)))
			pTurret->_EnterVehicle(me->GetVehicleKit(), 7);
		if (Creature *turret = CAST_CRE(me->GetVehicleKit()->GetPassenger(7)))
...
struct mob_proximity_mineAI : public ScriptedAI
{
    ...
    void MoveInLineOfSight(Unit *who)
    {
        if (!Boom && me->IsWithinDistInMap(who, 0.5f) && who->GetTypeId() == TYPEID_PLAYER)
        {
			...
			if (Creature *pMimiron = Creature::GetCreature((*me), pInstance->GetData64(TYPE_MIMIRON)))
				pMimiron->AI()->DoAction(STRIKE_BOMBE);
        }
...
struct boss_vx_001AI : public BossAI
{
    ...
    void EnterCombat(Unit *who)
    {
        ...
        events.ScheduleEvent(EVENT_RAPID_BURST_G, 5000, 0, PHASE_VX001_SOLO);
		events.ScheduleEvent(EVENT_RAPID_BURST_D, 7500, 0, PHASE_VX001_SOLO);
        events.ScheduleEvent(EVENT_PRE_LASER_BARRAGE, urand(35000, 40000), 0, PHASE_VX001_SOLO); // Not works in phase 4 :(
	//	events.ScheduleEvent(EVENT_LASER_BARRAGE_ASSEMBLED, urand(35000, 40000), 0, PHASE_VX001_ASSEMBLED);
        events.ScheduleEvent(EVENT_ROCKET_STRIKE, 20000);
        events.ScheduleEvent(EVENT_HEAT_WAVE, urand(8000, 10000), 0, PHASE_VX001_SOLO);
        events.ScheduleEvent(EVENT_HAND_PULSE, 5000, 0, PHASE_VX001_ASSEMBLED);
		events.ScheduleEvent(EVENT_HAND_PULSE_D, 7500, 0, PHASE_VX001_ASSEMBLED);
    }
    ...
    void DoAction(const int32 action)
    {
        switch(action)
        {
            ...
            case DO_VX001_ASSEMBLED:
                ...
                events.RescheduleEvent(EVENT_HAND_PULSE, 5000, 0, PHASE_VX001_ASSEMBLED);
				events.RescheduleEvent(EVENT_HAND_PULSE_D, 7500, 0, PHASE_VX001_ASSEMBLED);
			//	events.RescheduleEvent(EVENT_LASER_BARRAGE_ASSEMBLED, urand(35000, 40000), 0, PHASE_VX001_ASSEMBLED);
                if (MimironHardMode)
                {
                    DoCast(me, SPELL_EMERGENCY_MODE);
                ...
                switch(eventId)
                {
                    case EVENT_RAPID_BURST_G:
                        ...
                        if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
							...
							{
								TargetAngle = me->GetAngle(pTarget);
							    me->SetOrientation(TargetAngle);
                                DoCast(BurstTarget, RAID_MODE(SPELL_RAPID_BURST_10, SPELL_RAPID_BURST_25));
							}
                        events.RescheduleEvent(EVENT_RAPID_BURST_G, 5000, 0, PHASE_VX001_SOLO);
                        break;
					case EVENT_RAPID_BURST_D:
						me->GetMotionMaster()->Initialize();
					    if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
						{
							TargetAngle = me->GetAngle(pTarget);
							me->SetOrientation(TargetAngle);
                            DoCast(pTarget, RAID_MODE(SPELL_RAPID_BURST_D_10, SPELL_RAPID_BURST_D_25));
						}
                        events.RescheduleEvent(EVENT_RAPID_BURST_D, 5000, 0, PHASE_VX001_SOLO);
                        break;
                    ...
                    case EVENT_LASER_BARRAGE:
                        ...
                        break;
			/*		case EVENT_LASER_BARRAGE_ASSEMBLED:
							if (Creature *pLeviathan = Creature::GetCreature((*me), pInstance->GetData64(DATA_LEVIATHAN_MK_II)))
								pLeviathan->StopMoving();
                        DoCastAOE(SPELL_LASER_BARRAGE);
                        events.RescheduleEvent(EVENT_LASER_BARRAGE_ASSEMBLED, urand(35000, 40000));
                        break; */
                    case EVENT_ROCKET_STRIKE:
                        if (Unit *pTarget1 = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
						{
                            if (Creature *missile = CAST_CRE(me->GetVehicleKit()->GetPassenger(5)))
                                missile->CastSpell(pTarget1, SPELL_ROCKET_STRIKE, true);
						}
						if (Unit *pTarget2 = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
						{
                            if (Creature *missile2 = CAST_CRE(me->GetVehicleKit()->GetPassenger(6)))
                                missile2->CastSpell(pTarget2, SPELL_ROCKET_STRIKE, true);
						}
                        events.RescheduleEvent(EVENT_ROCKET_STRIKE, 15000);
                        break;
                    ...
                    case EVENT_HAND_PULSE:
                        ...
                        events.RescheduleEvent(EVENT_HAND_PULSE, 5000);
                        break;
					case EVENT_HAND_PULSE_D:
                        if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
						{
							TargetAngle = me->GetAngle(pTarget);
							me->SetOrientation(TargetAngle);
                            DoCast(pTarget, RAID_MODE(SPELL_HAND_PULSE_D_10, SPELL_HAND_PULSE_D_25));
						}
                        events.RescheduleEvent(EVENT_HAND_PULSE_D, 5000);
                        break;
...
struct rocket_strikeAI : public ScriptedAI
{
	...
	{
		if (pWho && pWho->GetTypeId() == TYPEID_PLAYER && pSpell->Id == 63041) // Rocket Strike Damage
			if (Creature *pMimiron = Creature::GetCreature((*me), pInstance->GetData64(TYPE_MIMIRON)))
				pMimiron->AI()->DoAction(STRIKE_BOMBE);
	}
    ...
    void MoveInLineOfSight(Unit *who)
    {
		if (Creature* who = me->FindNearestCreature(NPC_ASSAULT_BOT,0.5f,true))
     	   if (me->IsWithinDistInMap(who, 0.5f))
    	    {
  	         	who->DisappearAndDie();
...
public:
    ...
    struct mob_boom_botAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
			if (Unit* pWho = me->SelectVictim())
				if (me->GetDistance2d(pWho) <= 0.5)
                {
                    //Explosion
                    DoCast(pWho, 62834);

					if (Creature *pMimiron = Creature::GetCreature((*me), m_pInstance->GetData64(TYPE_MIMIRON)))
						pMimiron->AI()->DoAction(STRIKE_BOMBE);
                }
        }
    };
...
{
public:
	GOHello_not_push_button() : GameObjectScript("GOHello_not_push_button") {}
	...
	bool OnGossipHello(Player* pPlayer, GameObject* pGo)
	{
		...
		if (pInstance->GetBossState(TYPE_MIMIRON) == FAIL || pInstance->GetBossState(TYPE_MIMIRON) == NOT_STARTED)
			if (Creature *pMimiron = Creature::GetCreature((*pGo), pInstance->GetData64(TYPE_MIMIRON)))
			{
...
struct mob_mimiron_flameAI : public ScriptedAI
{
    mob_mimiron_flameAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        ...
        me->SetReactState(REACT_PASSIVE);
        DoCast(me, SPELL_FLAME, false); //true ?
        uiFlameTimer = 8000;
    }
