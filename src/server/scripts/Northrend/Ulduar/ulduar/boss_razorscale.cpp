...
//TODO: Harpoon event is automated needs to be checked but gob harpoon don't reset and don't cast spell...
...
#define GOSSIP_ITEM_1 "Nous sommes prets !"
...
enum DarkRuneSpells
{
    ...
    SPELL_WHIRLWIND                 = 63808, // 63807,
};

#define ACHIEVEMENT_QUICK_SHAVE     RAID_MODE(2919, 2921)
#define ACHIEVEMENT_DOWN_NAIN       RAID_MODE(2923, 2924)
    ...
    struct boss_razorscaleAI : public BossAI
    {
        ...
        void JustDied(Unit* /*Killer*/)
        {
            ...
            if (instance)
            {
                ...
                if (IronCount >= 25)
                    instance->DoCompleteAchievement(ACHIEVEMENT_DOWN_NAIN);
            }
        }
        ...
        void SpellHitTarget(Unit* target, const SpellEntry* spell)
        {
            if(target && target->GetEntry() == NPC_DARK_RUNE_GUARDIAN)
                if (spell->Id == SPELL_FLAMEBREATH)
                    if (target->isDead()) // Achievement need to kill NPC with flamebreath
                        IronCount++;
        }
            ...
            if (phase == PHASE_GROUND)
            {
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_FLIGHT:
                        ...
                        events.ScheduleEvent(EVENT_FIREBALL, 6000, 0, PHASE_FLIGHT);
                        events.ScheduleEvent(EVENT_DEVOURING, 6000, 0, PHASE_FLIGHT);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_DEVOURING:
                            ...
                            events.ScheduleEvent(EVENT_DEVOURING, 6000, 0, PHASE_PERMAGROUND);
                            return;
                        case EVENT_BUFFET:
                            DoCastAOE(SPELL_WINGBUFFET);
                            events.CancelEvent(EVENT_BUFFET);
