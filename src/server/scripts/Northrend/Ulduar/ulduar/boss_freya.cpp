...
#define EMOTE_GIFT                              "Un Don de la Lieuse-de-vie commence a grandir !"
#define EMOTE_ALLIES                            "Les allies de la nature viennent d'apparaitre !"
...
#define ACHIEV_ELDERS_TIME                      RAID_MODE(2979, 3118)
...
enum Spells
{
    ...
    SPELL_IRONBRANCHS_ESSENCE                   = 65586, // 62387,
...
// Ancient Conservator
#define SPELL_NATURES_FURY                      RAID_MODE(62589,63571)
...
public:
    ...
    struct boss_freyaAI : public BossAI
    {
        ...
        void JustDied(Unit * /*victim*/)
        {
			...
			me->setFaction(35);
			me->SetVisible(false);
			if (Creature* FreyaImage = me->SummonCreature(NPC_FREYA_IMAGE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN))
			{
				FreyaImage->SetVisible(true);
				FreyaImage->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
				FreyaImage->ForcedDespawn(15000);
				FreyaImage->Say(SAY_DEATH, 0, FreyaImage->GetGUID());
			}

			if (Creature* Brightleaf = me->GetCreature(*me, instance->GetData64(DATA_BRIGHTLEAF)))
                if (Brightleaf->isAlive())
                {
					Brightleaf->RemoveAllAuras();
                    Brightleaf->ForcedDespawn();
                }
            
            if (Creature* Ironbranch = me->GetCreature(*me, instance->GetData64(DATA_IRONBRANCH)))
                if (Ironbranch->isAlive())
                {
					Ironbranch->RemoveAllAuras();
                    Ironbranch->ForcedDespawn();
                }
            
            if (Creature* Stonebark = me->GetCreature(*me, instance->GetData64(DATA_STONEBARK)))
                if (Stonebark->isAlive())
                {
					Stonebark->RemoveAllAuras();
                    Stonebark->ForcedDespawn();
                }
                    ...
                    if (Elemental[0]->isDead())
                        if (Elemental[1]->isDead())
                            if (Elemental[2]->isDead())
                            {
                                ...
                                if (Creature* Freya = me->GetCreature(*me, instance->GetData64(TYPE_FREYA)))
...
public:
    ...
    struct npc_elder_brightleafAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*victim*/)
        {
			...
			if (pInstance)
				pInstance->SetData(FREYA_ADD_COUNT_DEATH, pInstance->GetData(FREYA_ADD_COUNT_DEATH)+1);
        }
...
public:
    ...
    struct npc_detonating_lasherAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*victim*/)
        {
            ...
            if (Creature* Freya = me->GetCreature(*me, pInstance->GetData64(TYPE_FREYA)))
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (uiSpawnHealthySporeTimer <= diff && healthySporesSpawned < 10)
            {
                for (uint32 i = 0; i < 2; ++i)
                {
					...
					if (Creature* spore = me->SummonCreature(NPC_HEALTHY_SPORE, pos, TEMPSUMMON_TIMED_DESPAWN, 20000))
						spore->SetInCombatWithZone();
                }
...
public:
    ...
    {
        npc_healthy_sporeAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
		...
		void EnterCombat(Unit* /*pWho*/)
        {
			...
			{
                me->SetInCombatWith(Freya);
				Freya->SetInCombatWith(me);
                me->AddThreat(Freya, 50.0f);
			}
        }
    };
};
