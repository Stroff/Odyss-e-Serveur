...
#define SPELL_POWERING_UP         RAID_MODE(67590, 67602, 67603, 67604)
#define SPELL_EMPOWERED_DARK      RAID_MODE(65724, 67213, 67214, 67215)
#define SPELL_EMPOWERED_LIGHT     RAID_MODE(65748, 67216, 67217, 67218)
...
public:
    ...
    {
        mob_essence_of_twinAI(Creature* pCreature) : ScriptedAI(pCreature) 
        {
        }
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (m_uiRangeCheckTimer < uiDiff)
            {
                ...
                for(Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                {
                   Unit* pPlayer = itr->getSource();
                   if (!pPlayer) continue;
                        if (pPlayer->isAlive() && pPlayer->IsWithinDistInMap(me, 4.0f))
                        {
                            if (pPlayer->HasAura(SPELL_DARK_ESSENCE))
                            {
                                // from what I've been told, you get from 6 to 8 stacks at once when approaching an unleashed ball
                                int timesToCast = urand(6, 8); 
                                for(int i = 0; i < timesToCast; ++i)
                                    pPlayer->CastSpell(pPlayer, SPELL_POWERING_UP, true) ;

                                Aura *Stack = pPlayer->GetAura(SPELL_POWERING_UP);
                                if (Stack && Stack->GetStackAmount() > 99) {
                                    pPlayer->CastSpell(pPlayer, SPELL_EMPOWERED_DARK, true) ; 
                            ...
                            {
                                DoCast(me, SPELL_UNLEASHED_DARK);
                ...
                for(Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                {
                   ...
                   if (pPlayer->isAlive() && pPlayer->IsWithinDistInMap(me, 4.0f))
                   {
                          if (pPlayer->HasAura(SPELL_LIGHT_ESSENCE))
                          {
                              ...
                              Aura *Stack = pPlayer->GetAura(SPELL_POWERING_UP);
                              if (Stack && Stack->GetStackAmount() > 97)
                              {
                                  pPlayer->CastSpell(pPlayer, SPELL_EMPOWERED_LIGHT, true) ; 
                                  pPlayer->RemoveAurasDueToSpell(SPELL_POWERING_UP);
                              }
                              me->ForcedDespawn(200);
                          }
                          ...
                          {
                              DoCast(me, SPELL_UNLEASHED_LIGHT);
                              me->ForcedDespawn(500);
                          }
                    }
                }
