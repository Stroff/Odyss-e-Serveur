...
#define  NPC_SNOBOLD_VASSAL   RAID_MODE(34800, 35441, 35442, 35443)
...
public:
    ...
    struct mob_snobold_vassalAI : public ScriptedAI
    {
        ...
        void EnterCombat(Unit *pWho)
        {
            ...
            me->TauntApply(pWho);
            me->AddAura(SPELL_SNOBOLLED, pWho);
        }
            ...
            if (pTarget)
            {
                if (!pTarget->isAlive())
                {
                    if (m_pInstance)
                    {
                        ...
                        else if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0, true))
                        {
