...
public:
        ...
        void OnGameObjectCreate(GameObject* pGO)
    {
        ...
        if (pGO->GetEntry() == GO_CACHE_Of_EREGOS)
        {
            m_uiCacheOfEregosGUID = pGO->GetGUID() ;
        }
    }
    ...
    void SetData(uint32 type, uint32 data)
    {
        switch(type)
        {
            ...
            case DATA_EREGOS_EVENT:
                ...
                if (data == DONE)
                    if (GameObject* pGO = instance->GetGameObject(m_uiCacheOfEregosGUID))
                    {
                        pGO->SetRespawnTime(30000) ;
                        pGO->Respawn() ;
                    }
                break;
