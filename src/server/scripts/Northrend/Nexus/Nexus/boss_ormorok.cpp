...
{
    boss_ormorokAI(Creature *c) : ScriptedAI(c), summons(me)
    {
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new mob_crystalline_frayerAI (pCreature);
    }

struct mob_crystalline_frayerAI : public ScriptedAI
{
    mob_crystalline_frayerAI(Creature* c) : ScriptedAI(c) {}
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (ensare_timer <= diff)
            DoCast(48053);
        else ensare_timer -= diff;
    }
};
};
