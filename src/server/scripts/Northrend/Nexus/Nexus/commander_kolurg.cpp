...
#define CREATURE_HORDE_CLERIC                                26803
#define CREATURE_COMMANDER_KOLURG                            26798
...
{
    boss_commander_kolurgAI(Creature *c) : ScriptedAI(c), summons(me)
    {
    ...
    void Reset() 
    {
        ...
        me->SummonCreature(CREATURE_HORDE_CLERIC, me->GetPositionX()+3, me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_DEAD_DESPAWN) ;
        me->SummonCreature(CREATURE_HORDE_CLERIC, me->GetPositionX()-3, me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_DEAD_DESPAWN) ;
        if (pInstance)
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_horde_clericAI (pCreature);
    }

struct npc_horde_clericAI : public ScriptedAI
{
    npc_horde_clericAI(Creature *c) : ScriptedAI(c) 
    {
        pInstance = c->GetInstanceScript();
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            if(Unit* target = me->FindNearestCreature(CREATURE_COMMANDER_KOLURG, 40, true))
            {
