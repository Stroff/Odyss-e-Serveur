...
public:
    ...
    struct npc_DeathspeakerServantAI : public ScriptedAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_CHAOS_BOLT, 4000);
            events.ScheduleEvent(EVENT_CONSUMING_SHADOWS, 8000);
            events.ScheduleEvent(EVENT_CURSE_OF_AGONY, 11000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_CHAOS_BOLT:
                        if(Unit * pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 30.f, true))
                            DoCast(pTarget, SPELL_CHAOS_BOLT);
                        events.ScheduleEvent(EVENT_CHAOS_BOLT, 4000);
                        break;
                    case EVENT_CONSUMING_SHADOWS:
                        if(Unit * pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.f, true))
                            DoCast(pTarget, SPELL_CONSUMING_SHADOWS);
                        events.ScheduleEvent(EVENT_CONSUMING_SHADOWS, 10000);
                        break;
                    case EVENT_CURSE_OF_AGONY:
                        if(Unit * pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 30.f, true))
                            DoCast(pTarget, SPELL_CURSE_OF_AGONY);
                        events.ScheduleEvent(EVENT_CURSE_OF_AGONY, 10000);
                        break;
                    default:
                        break;
                }
...
enum discipleEvents {
    ...
    EVENT_SHADOW_BOLT_DISCIPLE = 2,
    EVENT_SHADOW_MEND = 3
};

enum discipleSpells {
    SPELL_DARK_BLESSING           = 69391,
    SPELL_SHADOW_BOLT_DISCIPLE    = 69387,
...
public:
    ...
    struct npc_DeathspeakerDiscipleAI : public ScriptedAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_DARK_BLESSING, 8000);
            events.ScheduleEvent(EVENT_SHADOW_BOLT_DISCIPLE, 4000);
            events.ScheduleEvent(EVENT_SHADOW_MEND, 11000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_DARK_BLESSING:
                        sLog->outString("entered dark blessing");
                        if(Unit * pTarget = SelectRandomFriendlyMissingBuff(SPELL_DARK_BLESSING)) {
                            sLog->outString("cast dark blessing");
                            DoCast(pTarget, SPELL_DARK_BLESSING);
                        }
                        events.ScheduleEvent(EVENT_DARK_BLESSING, 10000);
                        break;
                    case EVENT_SHADOW_BOLT_DISCIPLE:
                        if(Unit * pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 40.f, true))
                            DoCast(pTarget, SPELL_SHADOW_BOLT_DISCIPLE);
                        events.ScheduleEvent(EVENT_SHADOW_BOLT_DISCIPLE, 4000);
                        break;
                    case EVENT_SHADOW_MEND:
                        if(Unit * pTarget = DoSelectLowestHpFriendly(40.0f))
                            DoCast(pTarget, SPELL_SHADOW_MEND);
                        events.ScheduleEvent(EVENT_SHADOW_MEND, 10000);
                        break;
                    default:
                        break;
                }
...
enum zealotEvents {
    EVENT_SHADOW_CLEAVE_ZEALOT = 1,
};

enum zealotSpells {
    SPELL_SHADOW_CLEAVE_ZEALOT  = 69492
};
...
public:
    ...
    struct npc_DeathspeakerZealotAI : public ScriptedAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_SHADOW_CLEAVE_ZEALOT, 5000);
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SHADOW_CLEAVE_ZEALOT:
                        DoCastVictim(SPELL_SHADOW_CLEAVE_ZEALOT);
                        events.ScheduleEvent(EVENT_SHADOW_CLEAVE_ZEALOT, 8000);
                        break;
                    default:
                        break;
                }
...
enum attedantEvents {
    EVENT_SHADOW_BOLT_ATTEDANT = 1,
    EVENT_SHADOW_NOVA = 2
};

enum attedantSpells {
    SPELL_SHADOW_BOLT_ATTEDANT  = 69387,
...
public:
    ...
    struct npc_DeathspeakerAttedantAI : public ScriptedAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_SHADOW_BOLT_ATTEDANT, 4000);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SHADOW_BOLT_ATTEDANT:
                        if(Unit * pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 40.f, true))
                            DoCast(pTarget, SPELL_SHADOW_BOLT_ATTEDANT);
                        events.ScheduleEvent(EVENT_SHADOW_BOLT_ATTEDANT, 4000);
                        break;
