...
public:
    ...
    struct npc_NerubarBroodkeeperAI : public ScriptedAI
    {
        ...
        npc_NerubarBroodkeeperAI(Creature *c) : ScriptedAI(c), summons(c)
        {
        }
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            events.ScheduleEvent(EVENT_SUMMON_SCARABS, 8000);
            events.ScheduleEvent(EVENT_WEB_WRAP, 11000);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SUMMON_SCARABS:
                        DoCast(SPELL_SUMMON_SCARABS);
                        events.ScheduleEvent(EVENT_SUMMON_SCARABS, 10000);
                        break;
                    case EVENT_WEB_WRAP:
                        if(Unit * pTarget = SelectTarget(SELECT_TARGET_RANDOM, 1, 40.f, true))
                            DoCast(pTarget, SPELL_WEB_WRAP);
                        events.ScheduleEvent(EVENT_WEB_WRAP, 10000);
                        break;
                    case EVENT_DARK_MENDING:
                        if(Unit * pTarget = DoSelectLowestHpFriendly(40.0f))
                            DoCast(pTarget, SPELL_DARK_MENDING);
                        events.ScheduleEvent(EVENT_DARK_MENDING, 10000);
                        break;
                    default:
                        break;
                }
