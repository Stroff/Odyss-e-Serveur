    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (uiSummonEtherealSphere_Timer < uiDiff)
        {
            ...
            if (IsHeroic()) // extra one for heroic
                me->SummonCreature(NPC_ETHEREAL_SPHERE, me->GetPositionX()-5+rand()%10, me->GetPositionY()-5+rand()%10, me->GetPositionZ()+3, 0, TEMPSUMMON_TIMED_DESPAWN, 40000);
                ...
                if (!PlayerList.isEmpty())
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                        if (i->getSource()->isAlive())
                            DoTeleportPlayer(i->getSource(), me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()+3, i->getSource()->GetOrientation());
            }
