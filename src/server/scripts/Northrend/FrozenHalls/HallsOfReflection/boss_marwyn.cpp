...
public:
    ...
    struct boss_marwynAI : public ScriptedAI
    {
       ...
       void Reset()
       {
          ...
          if(m_pInstance->GetData(TYPE_MARWYN) != FAIL) {
              SummonCount = 0;
              m_bIsCall = false;
              m_failDone = false;
       ...
       void UpdateAI(const uint32 uiDiff)
        {
            ...
            else if(m_pInstance->GetData(TYPE_MARWYN) == FAIL && !m_failDone) {
                SummonCount--;
                m_uiCheckSummon -= 4;
            ...
            {
                DoCast(me->getVictim(), SPELL_OBLITERATE);
            ...
            if (m_uiWellTimer < uiDiff) 
            {
                ...
                m_uiWellTimer= urand(18000, 23000);
            ...
            if (m_uiSharedSufferingTimer < uiDiff) 
            {
                ...
                m_uiSharedSufferingTimer = urand(13000, 18000);
            ...
            if (m_uiFleshTimer < uiDiff) 
            {
                ...
                m_uiFleshTimer = urand(10000, 15000);
