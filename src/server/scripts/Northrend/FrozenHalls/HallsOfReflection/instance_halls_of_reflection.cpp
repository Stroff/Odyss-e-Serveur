...
public:
    ...
    struct instance_halls_of_reflection_InstanceMapScript : public InstanceScript
    {
        ...
        {
            uint32 pGroupNumber = 1;
            uint32 pInstanceNumber = 0;
            if(player->GetGroup())
                pGroupNumber = player->GetGroup()->GetMembersCount();
            
            Map::PlayerList const &lPlayers = instance->GetPlayers();
            for (Map::PlayerList::const_iterator itr = lPlayers.begin(); itr != lPlayers.end(); ++itr)
                pInstanceNumber++;

            if(pGroupNumber == pInstanceNumber) {
                if (GetData(TYPE_FALRIC) == FAIL) {
                    events.ScheduleEvent(EVENT_FALRIC_BACK, 40000);
                    OpenDoor(m_uiExitGateGUID);
                }
                else if (GetData(TYPE_MARWYN) == FAIL) {
                    events.ScheduleEvent(EVENT_MARWYN_BACK, 40000);
                    OpenDoor(m_uiExitGateGUID);
                }
            }
        }
        ...
        void Update(uint32 diff) {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_FALRIC_BACK:
                        SetData(TYPE_FALRIC, SPECIAL);
                        break;  
                    case EVENT_MARWYN_BACK:
