...
enum Events
{
    ...
    EVENT_HOARFROST         = 6,
    EVENT_ICY_BLAST         = 7,
    EVENT_ICY_BLAST_AURA    = 8,
    EVENT_ICY_BLAST_AOE     = 9,


    EVENT_INTRO_1           = 10,
    EVENT_INTRO_2           = 11,
    EVENT_INTRO_3           = 12,
    EVENT_COMBAT_START      = 13,
};
...
enum Actions
{
    ...
    ACTION_END_RIMEFANG     = 4
};

enum Moves
{
    MOVE_TYRANNUS_GROUND   = 5,
    MOVE_RIMEFANG_END      = 6
};
...
static const Position sylvanas_jainaOutroPos = {1077.5408f, 155.9121f, 628.1554f, 3.3818f};
static const Position miscPos = {1018.376f, 167.2495f, 628.1561f, 0.000000f};   //tyrannus combat start position
static const Position rimefangEndPosition = {1307.78f, 51.59f, 810.622, 0.0f};
...
class boss_tyrannus : public CreatureScript
{
    public:
        ...
        struct boss_tyrannusAI : public BossAI
        {
            ...
            {
                if (events.GetPhaseMask() == PHASE_NONE || !victim)
                    return;
            ...
            void JustDied(Unit* /*killer*/)
            {
                if (instance->GetData(DATA_TEAM_IN_INSTANCE) == ALLIANCE)
                    me->SummonCreature(NPC_JAINA_PART2, sylvanas_jainaOutroPos, TEMPSUMMON_CORPSE_DESPAWN);
                else
                    me->SummonCreature(NPC_SYLVANAS_PART2, sylvanas_jainaOutroPos, TEMPSUMMON_CORPSE_DESPAWN);
                ...
                if (Creature* rimefang = me->GetCreature(*me, instance->GetData64(DATA_RIMEFANG)))
                    rimefang->AI()->DoAction(ACTION_END_RIMEFANG);    // rimefang flies away before unsummoning
            }

            void DoAction(const int32 actionId)
            {
                if (actionId == ACTION_START_INTRO && events.GetPhaseMask() == PHASE_NONE)
                {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_INTRO_3:
                            me->ExitVehicle();
                            me->GetMotionMaster()->MovePoint(MOVE_TYRANNUS_GROUND, miscPos);
                            break;
                        case EVENT_OVERLORD_BRAND:
                            if (Unit *target = SelectTarget(SELECT_TARGET_RANDOM, 1, 100, true))
                                DoCast(target, SPELL_OVERLORD_BRAND);
                            events.ScheduleEvent(EVENT_OVERLORD_BRAND, urand(14000, 16000));
                            break;
    ...
    public:
        ...
        struct boss_rimefangAI : public ScriptedAI
        {
            ...
            void DoAction(const int32 actionId)
            {
                ...
                else if(actionId == ACTION_END_RIMEFANG)
                {
                    me->SetSpeed(MOVE_FLIGHT, 3.5f, true);
                    _events.SetPhase(PHASE_OUTRO);
                    me->GetMotionMaster()->MovePoint(MOVE_RIMEFANG_END, rimefangEndPosition);
                }
            }
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                if (id != MOVE_RIMEFANG_END)
                    return;

                // Rimefang reached end position, now unsummon him
                me->DespawnOrUnsummon();
            }
            ...
            Unit* GetTriggerTarget()
            {
                ...
                {
                    Position PosTri;
                    pTarget->GetNearPosition(PosTri, 3.f, float(urand(0,360)));
...
public:
    ...
    struct mob_eisige_luft_raufang_icc5AI: public ScriptedAI
    {
        ...
        {
            events.Reset();
            events.ScheduleEvent(EVENT_ICY_BLAST_AURA, 3800);
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    case EVENT_ICY_BLAST_AURA:
                        DoCast(me, SPELL_ICY_BLAST_AURA);
                        break;
                }
            }
        }
