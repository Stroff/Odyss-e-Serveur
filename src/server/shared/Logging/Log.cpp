...
Log::Log() :
    raLogfile(NULL), logfile(NULL), gmLogfile(NULL), simpleLogfile(NULL), charLogfile(NULL),
...
void Log::Initialize()
{
	...
	simpleLogfile = openLogFile("SimpleLogFile", NULL, "a");
...
void Log::outSimple(const char * str, ...)
{
    if (!str)
        return;
    ...
    if (simpleLogfile)
    {
        outTimestamp(simpleLogfile);
        va_list ap;
        va_start(ap, str);
        vfprintf(simpleLogfile, str, ap);
        fprintf(simpleLogfile, "\n" );
        va_end(ap);
        fflush(simpleLogfile);
    }
